...
if curl -s http://localhost:$PORT_SERVICE_PORTFOLIO/health | grep -q "OK"; then
...
if curl -s http://localhost:$PORT_GATEWAY_MAIN/health | grep -q "OK"; then
    echo -e "${GREEN}✓ Running${NC}"
else
    echo -e "${RED}✗ Failed${NC}"
    cat /tmp/gateway.log
...
DIRECT_RESPONSE=$(curl -s http://localhost:$PORT_SERVICE_PORTFOLIO/api/v1/stocks/trending)
...
GATEWAY_RESPONSE=$(curl -s http://localhost:$PORT_GATEWAY_MAIN/api/v1/stocks/trending)
...
SEARCH_RESPONSE=$(curl -s "http://localhost:$PORT_GATEWAY_MAIN/api/v1/stocks/search?q=AAPL")
...
echo "📊 Summary:"
echo "-----------"
echo "Portfolio service: http://localhost:$PORT_SERVICE_PORTFOLIO"
echo "Gateway service: http://localhost:$PORT_GATEWAY_MAIN"
echo "Dashboard: http://localhost:$PORT_GATEWAY_MAIN/dashboard.html"
echo ""
echo "Logs:"
