...
export default function NotificationSettings({ user, tabId, onSave, saving }: NotificationSettingsProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.emailPreferences) {
          setEmailPreferences(prev => ({ ...prev, ...data.emailPreferences }));
        }
      }
    } catch (error) {
  ...
  const toggleChannel = (typeId: string, channel: keyof NotificationChannel) => {
    setNotificationTypes(prev => 
      prev.map(type => 
        type.id === typeId 
          ? { ...type, channels: { ...type.channels, [channel]: !type.channels[channel] } }
          : type
      )
    );
  };
  ...
  const toggleAllChannels = (channel: keyof NotificationChannel, enabled: boolean) => {
    setNotificationTypes(prev => 
      prev.map(type => ({
        ...type,
  ...
  const sendTestNotification = async () => {
    try {
      const response = await authClient.fetch('/api/settings/notifications/test', {
        method: 'POST',
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => {
                setGlobalSettings({ ...globalSettings, pushEnabled: e.target.checked });
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => {
                setGlobalSettings({ ...globalSettings, smsEnabled: e.target.checked });
            ...
            <div key={type.id} className="bg-white border border-gray-200 rounded-lg p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const allEnabled = Object.values(type.channels).every(v => v);
                    toggleAllForType(type.id, !allEnabled);
                  }}
                  className="text-xs text-indigo-600 hover:text-indigo-700"
                >
                  {Object.values(type.channels).every(v => v) ? 'Disable all' : 'Enable all'}
                </button>
              </div>
        ...
        <div className="space-y-4">
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setGlobalSettings({ ...globalSettings, doNotDisturb: e.target.checked })}
              className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
            <span className="ml-2 text-sm text-gray-700">Enable Do Not Disturb mode</span>
          </label>
            ...
            <div className="ml-6 grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setGlobalSettings({ ...globalSettings, doNotDisturbStart: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setGlobalSettings({ ...globalSettings, doNotDisturbEnd: e.target.value })}
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
              </div>
            </div>
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setGlobalSettings({ ...globalSettings, weekendNotifications: e.target.checked })}
              className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
            <span className="ml-2 text-sm text-gray-700">Receive notifications on weekends</span>
          </label>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-medium text-gray-700">Email digest</label>
            <select
              value={globalSettings.emailDigest}
              onChange={(e) => setGlobalSettings({ ...globalSettings, emailDigest: e.target.value })}
              className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setEmailPreferences({ ...emailPreferences, format: e.target.value })}
              className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            >
        ...
        <div className="mt-4 space-y-3">
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setEmailPreferences({ ...emailPreferences, includeImages: e.target.checked })}
              className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
            <span className="ml-2 text-sm text-gray-700">Include images in emails</span>
          </label>
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setEmailPreferences({ ...emailPreferences, replyToNotifications: e.target.checked })}
              className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
            <span className="ml-2 text-sm text-gray-700">Allow replying to notification emails</span>
          </label>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setGlobalSettings({ ...globalSettings, soundEnabled: e.target.checked })}
              className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
          ...
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setGlobalSettings({ ...globalSettings, vibrationEnabled: e.target.checked })}
              className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
            />
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setGlobalSettings({ ...globalSettings, notificationSound: e.target.value })}
