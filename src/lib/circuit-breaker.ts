...
export class CircuitBreaker {
  ...
  canAttempt(): boolean {
    ...
    if (timeSinceLastAttempt < this.config.connectionCooldown) {
      console.log(`[Circuit Breaker] Cooldown period active, ${this.config.connectionCooldown - timeSinceLastAttempt}ms remaining`);
      return false;
    }
  ...
  recordFailure(): void {
    ...
    if (this.state === 'half-open') {
      ...
      this.state = 'open';
      console.log('[Circuit Breaker] Failed in half-open state, reopening circuit');
  ...
  getRetryDelay(): number {
    ...
    const multiplier = Math.pow(this.config.backoffMultiplier, this.failures - 1);
  ...
  getState(): {
    ...
    const timeSinceLastAttempt = now - this.lastAttemptTime;
    const cooldownRemaining = Math.max(0, this.config.connectionCooldown - timeSinceLastAttempt);
