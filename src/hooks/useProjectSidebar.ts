...
export function useProjectSidebar() {
  ...
  const [sidebarSettings, setSidebarSettings] = useState<SidebarSettings | null>(null);
  ...
  const updateSettings = useCallback(async (updates: Partial<SidebarSettings>) => {
    try {
      const response = await fetch('/api/workspace/sidebar/settings', {
        method: 'PUT',
        headers: {
  ...
  const updateProjectPreference = useCallback(async (
    projectId: string,
