...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sessionId) {
      ...
      const logs = await workspaceTerminalLogger.getSessionLogs(sessionId, limit);
      return NextResponse.json({
        success: true,
        type: 'session_logs',
        sessionId,
      ...
      const sessions = await workspaceTerminalLogger.getProjectSessions(projectId, active);
      const commands = await workspaceTerminalLogger.getProjectCommands(projectId, limit);
