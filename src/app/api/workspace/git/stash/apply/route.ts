...
export async function POST(request: NextRequest) {
  try {
    ...
    const { projectId, stashId, pop = false } = body;
    ...
    await gitService.applyStash(stashId, pop);
    
    return NextResponse.json({ 
      success: true,
      message: pop ? `Popped stash ${stashId}` : `Applied stash ${stashId}`,
      stashId,
      action: pop ? 'pop' : 'apply'
    });
  } catch (error) {
