...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!token) {
      console.log(`[Git Status API] 🚫 No accessToken cookie found for project: ${projectId}`);
    ...
    const { allowed, remaining, resetTime } = gitStatusRateLimiter.check(rateLimitKey);
    console.log(`[Git Status API] 📊 Rate limit check - allowed: ${allowed}, remaining: ${remaining}`);
    
    if (!allowed) {
      console.warn(`[Git Status API] 🛑 Rate limited for project: ${projectId}`);
