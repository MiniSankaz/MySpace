...
try {
  ...
  inMemoryTerminalService =
    tsModule.inMemoryTerminalService ||
    tsModule.InMemoryTerminalService.getInstance();
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!sessionToken) {
      console.log(
        "[Terminal API] No auth token found, allowing cleanup anyway",
      );
    ...
    const closedCount =
      inMemoryTerminalService.cleanupProjectSessions(projectId);

    console.log(
      `[Terminal API] Cleaned up ${closedCount} terminal sessions for project ${projectId}`,
    );
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to cleanup terminal sessions",
      },
      { status: 500 },
    );
  }
