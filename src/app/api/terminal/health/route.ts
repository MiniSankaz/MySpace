...
try {
  ...
  console.error(
    "[Terminal Health API] Failed to load terminal service:",
    error,
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const [projectId, count] of sessionsByProject.entries()) {
      if (count > 100) {
        issues.push(
          `Project ${projectId} has ${count} sessions (LOOP DETECTED)`,
        );
      } else if (count > 50) {
    ...
    const isHealthy =
      !hasLoops && memoryUsageMB.heapUsed < 4096 && allSessions.length < 100;
    ...
    return NextResponse.json({
      ...
      warnings: [
        ...
        ...(memoryUsageMB.heapUsed > process.env.PORT || 3000
