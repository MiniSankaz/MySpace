...
import { checkApiAuth, checkRateLimit, sanitizeResponse } from '@/middleware/auth-check';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = {
      ...
      performance: {
        avgResponseTime: orchestratorStatus.services.metrics?.averages?.responseTime || 0,
        errorRate: orchestratorStatus.services.metrics?.averages?.errorRate || 0
      },
      timestamp: new Date().toISOString()
    };
