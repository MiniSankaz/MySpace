...
try {
  ...
  console.log(
    "[Terminal Status API] Falling back to TypeScript terminal-memory service",
  );
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session) {
      ...
      console.log(
        `[Terminal Status API] Available sessions: ${allSessions.map((s: any) => s.id).join(", ")}`,
      );
      return NextResponse.json({ error: "Session not found" }, { status: 404 });
    }

    console.log(
      `[Terminal Status API] Found session ${sessionId}, status: ${session.status}`,
    );
