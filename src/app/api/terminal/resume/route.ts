...
try {
  ...
  console.error(
    "[Terminal Resume API] Failed to load terminal service:",
    error,
  );
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result =
      await inMemoryTerminalService.resumeProjectSessions(projectId);

    console.log(
      `[Terminal Resume API] Resumed ${result.sessions.length} sessions for project ${projectId}`,
    );
