...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = {
      ...
      recommendations: generateRecommendations(migrationStatus, orchestratorStatus)
    };
 ...
 */
function generateRecommendations(migrationStatus: any, orchestratorStatus: any): string[] {
  ...
  if (migrationStatus.mode === 'progressive') {
    const enabledCount = Object.values(migrationStatus.featureFlags)
    ...
    if (enabledCount === 0) {
      ...
      recommendations.push(`Migration progress: ${enabledCount}/${totalCount} features enabled`);
    } else {
