...
function WorkspacePageContent() {
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      ...
      const projectsWithTerminals = Object.keys(projectSessions).filter(
        (projectId) => {
          const sessions = projectSessions[projectId];
          return (
            sessions &&
            (sessions.system.length > 0 || sessions.claude.length > 0)
          );
        },
      );
      ...
      if (projectsWithTerminals.length > 0) {
        ...
        event.returnValue =
          "You have active terminal sessions. Leaving will close all terminals. Are you sure?";
    ...
    const handleUnmount = () => {
      ...
      if (totalTerminals > 0) {
        console.log(
          `[WorkspacePage] Closing ${totalTerminals} terminal sessions across ${projectsWithTerminals.length} projects`,
        );
