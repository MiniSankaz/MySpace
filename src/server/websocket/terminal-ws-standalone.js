// Try to load terminal config with ts-node support
let terminalConfig, getWebSocketUrl;
try {
  // Try with ts-node first
  require("ts-node/register");
  const config = require("../../config/terminal.config.ts");
  terminalConfig = config.terminalConfig || config.default;
  getWebSocketUrl = config.getWebSocketUrl;
} catch (error) {
  // Fallback to inline config if ts-node fails
  console.warn(
    "Failed to load terminal.config.ts, using fallback config:",
    error.message,
  );
  terminalConfig = {
    websocket: {
    ...
    memory: {
      ...
      rssEmergencyThreshold: parseInt(
        process.env.MEMORY_RSS_EMERGENCY || "6144",
      ),
      ...
      maxFocusedPerProject: parseInt(
        process.env.MAX_FOCUSED_PER_PROJECT || "10",
      ),
  ...
  getWebSocketUrl = function (type = "system") {
...
try {
  ...
  try {
    ...
    console.warn(
      "In-memory terminal service not available (optional):",
      error2.message,
    );
...
class TerminalWebSocketServer {
  constructor(port = terminalConfig.websocket.port) {
    ...
    this.memoryService = InMemoryTerminalService
      ? InMemoryTerminalService.getInstance()
      : null;
    ...
    if (this.memoryService) {
      this.memoryService.on("focusChanged", (data) => {
        ...
        console.log(
          `[Terminal WS] Focus event received: session=${sessionId}, focused=${focused}, project=${projectId}`,
        );
      ...
      this.memoryService.on("sessionSuspended", (data) => {
        ...
        console.log(
          `[Terminal WS] Session ${sessionId} suspended for project ${projectId}`,
        );
        ...
        if (session) {
          ...
          console.log(
            `[Terminal WS] Marked session ${sessionKey} as suspended, process kept alive`,
          );
        }
      });
    }
  ...
  cleanupInactiveSessions() {
    ...
    for (const [sessionKey, session] of this.sessions) {
      if (!session.ws || session.ws.readyState !== 1) {
        ...
        if (
          session.keepAliveUntil &&
          now > session.keepAliveUntil &&
          !session.suspended &&
          !session.disconnected
        ) {
          console.log(
            `Keep-alive expired for session ${sessionKey}, cleaning up`,
          );
  ...
  async handleConnection(ws, request) {
    ...
    try {
      ...
      let rawSessionId =
        ...
        `session_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;
      ...
      if (workingDir && fs.existsSync(workingDir)) {
        ...
        console.log(
          `⚠️ Invalid project path, using home directory: ${workingDir}`,
        );
      }
      ...
      if (session && session.process && !session.process.killed) {
        console.log(
          `Reusing existing session: ${sessionKey} (ID: ${sessionId})`,
        );
        ...
        console.log(
          `Session ${sessionKey} exists but process is dead. Creating new session.`,
        );
        this.sessions.delete(sessionKey);
        session = null;
      }
      ...
      if (!session) {
        ...
        const rateLimitKey =
          projectId || request.socket.remoteAddress || "global";
        if (!this.checkRateLimit(rateLimitKey)) {
          ...
          ws.send(
            JSON.stringify({
              type: "error",
              message:
                "Too many terminal sessions. Please wait a moment and try again.",
            }),
          );
        ...
        const getAvailableShell = () => {
          ...
          // Ultimate fallback
          console.warn(
            "⚠️ No standard shell found, using system default fallback",
          );
        ...
        if (this.memoryService && projectId && sessionId) {
          ...
          if (!memSession) {
            ...
            console.warn(
              `[Terminal WS] Session ${sessionId} not found in memory service, waiting...`,
            );
            ...
            setTimeout(() => {
              ...
              if (!memSession) {
                console.error(
                  `[Terminal WS] Session ${sessionId} still not found after wait`,
                );
                // As fallback, create it
                console.log(
                  `[Terminal WS] Creating fallback session ${sessionId}`,
                );
              ...
              console.log(
                `[Terminal WS] WebSocket registered for session ${sessionId}`,
              );
            }, 500); // Wait 500ms for Storage Service to create session
          } else {
            // Session exists, register immediately
            console.log(
              `[Terminal WS] Session ${sessionId} found, registering WebSocket`,
            );
            this.memoryService.registerWebSocketConnection(sessionId, ws);
            this.memoryService.markWebSocketReady(sessionId);
        ...
        shellProcess.onData((data) => {
          ...
          if (
            ...
            terminalConfig.memory.rssWarningThreshold
          ) {
        ...
        shellProcess.onExit(({ exitCode, signal }) => {
          console.log(
            `Shell process exited with code ${exitCode}, signal ${signal}`,
          );
        ...
        console.log(
          `Reconnecting to session: ${sessionKey} (ID: ${sessionId})`,
        );
        session.ws = ws;
      ...
      ws.on("message", (message) => {
        try {
          ...
          if (
            this.memoryService &&
            this.memoryService.isSessionSuspended(sessionId)
          ) {
          ...
          switch (data.type) {
            ...
            case "resume":
              ...
              if (this.memoryService) {
                const result =
                  this.memoryService.resumeProjectSessions(projectId);

                // Send buffered output if any
                const suspensionState =
                  this.memoryService.getSuspensionState(sessionId);
                if (
                  suspensionState &&
                  suspensionState.bufferedOutput.length > 0
                ) {
                  ws.send(
                    JSON.stringify({
                      type: "buffered",
              ...
              if (session.process) {
                ...
                if (this.loggingService && session.dbSessionId) {
                  ...
                  if (data.data === "\r" || data.data === "\n") {
                    ...
                    // Backspace
                    session.currentCommand = session.currentCommand.slice(
                      0,
                      -1,
                    );
      ...
      ws.on("close", (code, reason) => {
        ...
        const isCircuitBreakerClose =
          code >= process.env.PORT || (4000 && code <= 4099);

        if (isIntentionalClose) {
          // Intentional close - suspend session instead of killing
          console.log(
            `Intentional close for session ${sessionKey}, suspending session`,
          );
          ...
          if (session) {
            ...
            console.log(
              `Session ${sessionKey} marked as disconnected, process kept alive`,
            );
          }
          ...
          console.log(
            `Page refresh (code 1001) for session ${sessionKey}, keeping session alive for 1 minute`,
          );
          ...
          console.warn(
            `Circuit breaker close (code ${code}) for session ${sessionKey}, keeping session for recovery`,
          );
          ...
          console.log(
            `Unexpected close (code ${code}) for session ${sessionKey}, keeping process alive for reconnection`,
          );
  ...
  parseSessionId(rawId, projectId) {
    ...
    if (parts.length > 1 && parts[parts.length - 1] === projectId) {
      ...
      console.log(
        `Migrating legacy composite session ID: ${rawId} -> ${baseId}`,
      );
      return baseId;
    }
  ...
  handleFocusChange(sessionId, projectId, isFocused) {
    ...
    if (this.memoryService) {
      ...
      console.log(
        `Session ${sessionId} focus changed to ${isFocused}, total focused: ${focusedList.length}`,
      );
    } else {
      ...
      if (isFocused) {
        ...
        console.log(
          `Session ${sessionId} is now focused for project ${projectId}`,
        );
      } else {
        focusedSet.delete(sessionId);
        console.log(
          `Session ${sessionId} is now unfocused for project ${projectId}`,
        );
      }
    }
  }
  ...
  async registerWebSocketWithRetry(sessionId, ws, projectId, retryCount = 0) {
    try {
      ...
      this.memoryService.registerWebSocketConnection(sessionId, ws);
      console.log(
        `Successfully registered WebSocket for session ${sessionId} on attempt ${retryCount + 1}`,
      );

      // Mark WebSocket as ready for this session
      this.memoryService.markWebSocketReady(sessionId);
      console.log(
        `[Terminal WS] Marked WebSocket ready for session ${sessionId}`,
      );
      ...
      if (currentRetries >= this.retryConfig.maxRetries) {
        console.error(
          `Max retries (${this.retryConfig.maxRetries}) reached for session ${sessionId}:`,
          error.message,
        );
        this.registrationRetries.delete(sessionId);
        throw error;
      ...
      const delay = Math.min(
        this.retryConfig.baseDelay *
          Math.pow(this.retryConfig.factor, currentRetries),
        this.retryConfig.maxDelay,
      );

      console.warn(
        `Failed to register WebSocket for session ${sessionId}, retrying in ${delay}ms (attempt ${currentRetries + 1}/${this.retryConfig.maxRetries}):`,
        error.message,
      );
      ...
      return this.registerWebSocketWithRetry(
        sessionId,
        ws,
        projectId,
        currentRetries + 1,
  ...
  closeAllSessions() {
    ...
    this.sessions.forEach((session, sessionId) => {
      ...
      if (this.loggingService && session.dbSessionId) {
        this.loggingService.endSession(session.dbSessionId).catch((err) => {
          console.error(
            `Failed to end DB session ${session.dbSessionId}:`,
            err,
          );
        });
      }
  ...
  handleGitWebSocketConnection(ws, projectId) {
    ...
    ws.on("error", (error) => {
      console.error(
        `[Git WS] WebSocket error for project ${projectId}:`,
        error,
      );
    });
  ...
  async refreshGitStatus(projectId, projectPath) {
    try {
      ...
      const branches = this.parseGitBranches(
      ...
      console.error(
        `Failed to refresh git status for project ${projectId}:`,
        error,
      );
...
function setupShutdownHandlers(server) {
  ...
  if (!process._shutdownHandlersSet) {
    ...
    process.on("SIGTERM", () => {
      console.log(
        "[Terminal Server] Received SIGTERM signal. Shutting down gracefully...",
      );
      shutdown("SIGTERM");
    });
