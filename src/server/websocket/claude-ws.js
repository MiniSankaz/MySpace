...
class ClaudeWebSocketServer {
  constructor(serverOrPort) {
    ...
    // Support both HTTP server attachment and standalone port
    let wsOptions;
    if (typeof serverOrPort === "number") {
      // Standalone server on specific port
      wsOptions = {
        port: serverOrPort,
        verifyClient: (info) => {
    ...
    if (typeof serverOrPort === "number") {
      console.log(
        `Claude WebSocket server initialized on port ${serverOrPort}`,
      );
    } else {
  ...
  handleConnection(ws, req) {
    ...
    if (projectPath) {
      try {
        ...
        if (!fs.existsSync(projectPath)) {
          console.warn(
            `Claude WebSocket: Path does not exist: ${projectPath}, using cwd`,
          );
    ...
    console.log(
      `Claude WebSocket connected: ${sessionId} for project ${projectId}`,
    );
    ...
    const initializeSession = () => {
      if (!session) {
        try {
          session = this.claudeService.createStreamingSession(
            sessionId,
            projectPath,
          );

          // Add listener for Claude output
          cleanupListener = this.claudeService.addSessionListener(
            sessionId,
            (event) => {
        ...
        switch (data.type) {
          case "input":
            ...
            if (initializeSession()) {
              try {
                const parsedCommand = this.claudeService.parseCommand(
                  data.data,
                );
                this.claudeService.sendToSession(
                  sessionId,
          ...
          case "command":
            ...
            try {
              ...
              const response = await this.claudeService.executeCommand(
                parsedCommand.prompt,
                {
                  projectPath,
                  sessionId,
