...
class ShellManager {
  ...
  async detectAvailableShells() {
    ...
    if (this.availableShells.length === 0) {
      console.error('[ShellManager] No shells detected, trying emergency fallback...');
  ...
  async testShell(shellPath) {
    ...
    try {
      ...
      console.warn(`[ShellManager] Shell ${shellPath} test failed:`, error.message);
    }
      ...
      try {
        ...
        if (shellPath) {
          ...
          if (result.available) {
            ...
            console.log(`[ShellManager] Found emergency fallback: ${shellPath}`);
            break;
          }
        }
      } catch (error) {
  ...
  async spawnShell(options = {}) {
    ...
    // All shells failed
    throw new Error(`Failed to spawn any shell. Last error: ${lastError?.message}`);
  }
