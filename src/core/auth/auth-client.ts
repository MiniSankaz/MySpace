...
import { offlineStore, OfflineUser, OfflineSession } from '@/core/database/offline-store';
...
class AuthClient {
  ...
  async getUser(): Promise<User | null> {
    ...
    if (userStr) {
      try {
        ...
        console.error('[AuthClient] Failed to parse user from localStorage:', error);
      }
    }
  ...
  async isAuthenticated(): Promise<boolean> {
    ...
    if (developmentConfig.enableOfflineMode && developmentConfig.enableMockData) {
      return true;
    }
  ...
  async fetch(url: string, options: RequestInit = {}): Promise<Response> {
    ...
    if (!isAuth) {
      try {
        ...
        if (developmentConfig.enableMockData) {
          console.warn('[AuthClient] Authentication failed, continuing in mock mode');
        } else {
