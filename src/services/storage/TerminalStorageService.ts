...
import {
  ...
  TerminalSession as LegacySession,
} from "../terminal-memory.service";
...
export class TerminalStorageService extends EventEmitter {
  ...
  private async initialize(): Promise<void> {
    console.log(
      `[TerminalStorageService] กำลังเริ่มต้นในโหมด: ${this.compatibilityMode}`,
    );
  ...
  private setupEventForwarding(): void {
    ...
    this.storageProvider.on("session:created", (session) => {
      ...
      const isMigrated =
        session.metadata?.migratedFrom === "legacy" ||
      ...
      if (!this.syncInProgress.has(session.id) && !isMigrated) {
        this.legacyService.emit(
          "sessionCreated",
          this.convertToLegacy(session),
        );
      }
    });
    ...
    this.storageProvider.on("session:updated", ({ sessionId, changes }) => {
      if (!this.syncInProgress.has(sessionId)) {
        this.legacyService.emit("sessionStatusChanged", {
          id: sessionId,
          ...changes,
        });
      }
  ...
  public async createSession(
    ...
    switch (this.compatibilityMode) {
      case "legacy":
        ...
        const legacyOnlySession = this.legacyService.createSession(
          projectId,
          projectPath,
          userId,
          mode,
      ...
      case "storage":
        ...
        try {
          const legacyCompat = this.legacyService.createSession(
            projectId,
            projectPath,
            userId,
            mode,
          ...
          this.legacyService.updateSessionId(
            legacyCompat.id,
            storageOnlySession.id,
          );
          this.sessionIdMap.set(storageOnlySession.id, storageOnlySession.id);
        } catch (error) {
          console.warn(
            "[TerminalStorageService] Failed to create legacy compatibility session:",
            error,
          );
        } finally {
      ...
      default:
        ...
        const hybridLegacySession = this.legacyService.createSession(
          projectId,
          projectPath,
          userId,
          mode,
    ...
    switch (this.compatibilityMode) {
      ...
      default:
        ...
        const storageSessions =
          await this.storageProvider.listSessions(projectId);
        const legacyCount =
          this.legacyService.suspendProjectSessions(projectId);
  ...
  public async resumeProjectSessions(projectId: string): Promise<{
    ...
    switch (this.compatibilityMode) {
      ...
      default:
        ...
        // Merge results
        const allSessions = [
          ...storageResult.sessions,
          ...legacyResult.sessions,
        ];
  ...
  private async migrateExistingSessions(): Promise<void> {
    ...
    console.log(
      "[TerminalStorageService] กำลัง migrate sessions จากระบบเก่า...",
    );
    ...
    try {
      ...
      console.log(
        `[TerminalStorageService] Migration complete: ${migrated} migrated, ${skipped} skipped`,
      );

      if (allLegacySessions.length > MAX_MIGRATIONS) {
        console.warn(
          `[TerminalStorageService] Limited migration to ${MAX_MIGRATIONS} sessions out of ${allLegacySessions.length}`,
        );
      }
    } catch (error) {
  ...
  private async syncToStorage(legacySession: LegacySession): Promise<void> {
    ...
    if (this.migratedSessions.has(legacySession.id)) {
      console.log(
        `[TerminalStorageService] Session ${legacySession.id} already migrated, skipping`,
      );
      return;
    }
    ...
    if (attempts >= 3) {
      console.error(
        `[TerminalStorageService] Max migration attempts reached for ${legacySession.id}`,
      );
      return;
    }
    ...
    try {
      ...
      const duplicateSession = allSessions.find(
        (s) =>
      ...
      console.error(
        `[TerminalStorageService] ไม่สามารถ sync session ${legacySession.id}:`,
        error,
      );
    } finally {
  ...
  public async switchMode(
    mode: "legacy" | "storage" | "hybrid",
  ): Promise<void> {
    console.log(
      `[TerminalStorageService] กำลังเปลี่ยนจาก ${this.compatibilityMode} ไปเป็น ${mode}`,
    );
