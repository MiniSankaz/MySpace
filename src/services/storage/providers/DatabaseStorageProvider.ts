...
export class DatabaseStorageProvider extends BaseStorageProvider {
  ...
  private cache: Map<string, { session: TerminalSession; timestamp: number }> =
    new Map();
  ...
  public async createSession(
    ...
    return this.trackOperation("write", async () => {
      ...
      this.log(
        "info",
        `สร้าง session ${result.id} สำหรับ project ${params.projectId}`,
      );
      return result;
    });
  }
  ...
  public async resumeSession(sessionId: string): Promise<ResumeResult> {
    ...
    const bufferedOutput = JSON.parse(
      suspension.bufferedOutput as string,
    ) as any[];
  ...
  private mapSessionToDb(session: Partial<TerminalSession>): any {
    ...
    if (session.currentPath !== undefined)
      dbData.currentPath = session.currentPath;
    if (session.wsConnected !== undefined)
      dbData.wsConnected = session.wsConnected;
    if (session.metadata !== undefined) dbData.metadata = session.metadata;
    if (session.environment !== undefined)
      dbData.environment = session.environment;
