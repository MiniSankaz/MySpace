...
export class HybridStorageProvider extends BaseStorageProvider {
  ...
  // Conflict tracking
  private conflicts: Map<string, { local: any; remote: any; timestamp: Date }> =
    new Map();
  ...
  private async initialSync(): Promise<void> {
    try {
      ...
      if (dbSessions.length > MAX_INITIAL_SYNC) {
        this.log(
          "warn",
          `Limiting initial sync to ${MAX_INITIAL_SYNC} out of ${dbSessions.length} sessions`,
        );
      }
      ...
      this.log(
        "info",
        `Initial sync สำเร็จ: โหลด ${sessionsToSync.length} sessions`,
      );
    } catch (error) {
  ...
  public async createSession(
    ...
    const now = Date.now();
    const recentCreations = Array.from(
      this.creationTimestamps.entries(),
    ).filter(([key, time]) => key.startsWith(projectKey) && now - time < 60000);

    if (recentCreations.length >= this.MAX_CREATIONS_PER_MINUTE) {
      console.error(
        `[HybridStorageProvider] LOOP DETECTED: ${recentCreations.length} sessions created in last minute for project ${params.projectId}`,
      );
      throw new Error(
        "Too many sessions created. Possible infinite loop detected.",
      );
    }
  ...
  private async performSync(): Promise<void> {
    ...
    for (const item of batch) {
      try {
        ...
        if (item.retries >= 3) {
          ...
          this.log(
            "error",
            `Sync failed permanently for ${item.sessionId}`,
            error,
          );
          this.syncQueue.delete(item.id);
    ...
    this.emitStorageEvent("storage:sync-complete", {
      synced,
      conflicts: failed,
    });
  ...
  private async resolveConflict(sessionId: string): Promise<void> {
    ...
    this.log(
      "info",
      `แก้ไข conflict สำหรับ ${sessionId} ด้วย ${this.config.conflictResolution}`,
    );
  }
  ...
  public async getStorageInfo(): Promise<StorageInfo> {
    ...
    return {
      ...
      performance: {
        ...
        avgWriteTime:
          (localInfo.performance.avgWriteTime +
            dbInfo.performance.avgWriteTime) /
          2,
  ...
  public async healthCheck(): Promise<HealthStatus> {
    ...
    if (this.conflicts.size > 0) {
      issues.push(
        `มี conflicts ที่ยังไม่ได้แก้ไข: ${this.conflicts.size} รายการ`,
      );
    }
