...
export class LocalStorageProvider extends BaseStorageProvider {
  ...
  public async createSession(
    ...
    return this.trackOperation("write", async () => {
      ...
      const projectFocused =
        this.focusedSessions.get(params.projectId) || new Set();
      if (projectFocused.size >= this.maxFocusedPerProject) {
        console.log(
          `[LocalStorageProvider] Max focused sessions (${this.maxFocusedPerProject}) reached for project ${params.projectId}`,
        );
        // Don't create new session if we've hit the focus limit
        const existingSession = Array.from(this.sessions.values()).find(
          (s) => s.projectId === params.projectId && !s.isFocused,
        );
        if (existingSession) {
          console.log(
            `[LocalStorageProvider] Reusing existing unfocused session ${existingSession.id}`,
          );
      ...
      if (this.sessions.size >= (this.config.maxSessions || 50)) {
        ...
        if (!evicted) {
          console.error(
            "[LocalStorageProvider] Unable to evict sessions, at maximum capacity",
          );
      ...
      // Get tab name
      const projectCount =
        this.projectSessions.get(params.projectId)?.size || 0;
      ...
      this.log(
        "info",
        `Created session ${session.id} for project ${params.projectId}`,
      );
      ...
      if (projectSessionCount > 100) {
        console.error(
          `[LocalStorageProvider] WARNING: Project ${params.projectId} has ${projectSessionCount} sessions!`,
        );
      }
  ...
  public async setSessionFocus(
    ...
    if (focused) {
      ...
      if (
        projectFocused.size >= this.maxFocusedPerProject &&
  ...
  private async evictOldestSession(): Promise<boolean> {
    ...
    if (sessions.length > 0) {
      ...
      if (deleted) {
        this.log(
          "info",
          `Evicted session ${sessionToEvict.id} (status: ${sessionToEvict.status}, focused: ${sessionToEvict.isFocused})`,
        );
        return true;
      } else {
  ...
  private async saveToDisk(): Promise<void> {
    ...
    try {
      const data = {
        ...
        projectSessions: Array.from(this.projectSessions.entries()).map(
          ([k, v]) => [k, Array.from(v)],
        ),
        focusedSessions: Array.from(this.focusedSessions.entries()).map(
          ([k, v]) => [k, Array.from(v)],
        ),
  ...
  private async loadFromDisk(): Promise<void> {
    ...
    try {
      ...
      this.projectSessions = new Map(
        data.projectSessions.map(([k, v]: [string, string[]]) => [
          k,
          new Set(v),
        ]),
      );
      this.focusedSessions = new Map(
        data.focusedSessions.map(([k, v]: [string, string[]]) => [
          k,
          new Set(v),
        ]),
      );
      this.sessionCounters = new Map(data.sessionCounters);
      this.suspendedStates = new Map(
        data.suspendedStates.map(([k, v]: [string, any]) => [
          k,
          {
            ...v,
