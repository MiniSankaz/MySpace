 ...
 */
export abstract class BaseStorageProvider extends EventEmitter implements ITerminalStorageService {
   ...
   */
  protected applyQueryFilters(sessions: TerminalSession[], query: SessionQuery): TerminalSession[] {
   ...
   */
  protected applyListOptions(sessions: TerminalSession[], options?: ListOptions): TerminalSession[] {
    let result = [...sessions];
    
    // Filter inactive if needed
    if (!options?.includeInactive) {
      result = result.filter(s => s.status !== 'closed' && s.status !== 'error');
    }
   ...
   */
  protected calculateMetrics(sessionCount: number, memoryUsage?: number): {
    avgReadTime: number;
    avgWriteTime: number;
    cacheHitRate?: number;
  } {
    const avgReadTime = this.metrics.reads > 0 
      ? this.metrics.totalReadTime / this.metrics.reads 
      : 0;
    
    const avgWriteTime = this.metrics.writes > 0
  ...
  public abstract createSession(params: SessionCreateParams): Promise<TerminalSession>;
  public abstract getSession(sessionId: string): Promise<TerminalSession | null>;
  public abstract updateSession(sessionId: string, data: Partial<TerminalSession>): Promise<void>;
  public abstract deleteSession(sessionId: string): Promise<boolean>;
  public abstract listSessions(projectId: string, options?: ListOptions): Promise<TerminalSession[]>;
  public abstract bulkUpdate(updates: SessionUpdate[]): Promise<void>;
  public abstract bulkDelete(sessionIds: string[]): Promise<number>;
  public abstract setSessionFocus(sessionId: string, focused: boolean): Promise<void>;
