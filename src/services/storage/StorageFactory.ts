...
export class StorageFactory {
  private static providers: Map<StorageMode, ITerminalStorageService> = new Map();
  ...
  public static initialize(config?: Partial<StorageConfig>): void {
    ...
    const mode = (process.env.TERMINAL_STORAGE_MODE as StorageMode) || 
                 config?.mode || 
                 'LOCAL';
    ...
    this.config = {
      ...
      hybrid: {
        syncStrategy: (process.env.TERMINAL_SYNC_STRATEGY as 'immediate' | 'eventual' | 'manual') || 'eventual',
        syncInterval: parseInt(process.env.TERMINAL_SYNC_INTERVAL || '30000'),
        conflictResolution: (process.env.TERMINAL_CONFLICT_RESOLUTION as 'local-wins' | 'database-wins' | 'latest-wins') || 'latest-wins',
    ...
    console.log(`[StorageFactory] Initialized with mode: ${mode}`, {
      ...
      database: this.config.database ? { ...this.config.database, connectionString: '***' } : undefined,
    ...
    switch (mode) {
      ...
      case 'DATABASE':
        if (!this.config.database?.connectionString) {
          throw new Error('DATABASE mode requires DATABASE_URL environment variable');
        }
        return new DatabaseStorageProvider(this.config.database);

      case 'HYBRID':
        if (!this.config.database?.connectionString) {
          throw new Error('HYBRID mode requires DATABASE_URL environment variable');
        }
  ...
  public static async switchMode(
    ...
    console.log(`[StorageFactory] Switching from ${this.currentMode} to ${newMode} mode`);
    ...
    if (migrateData && currentProvider) {
      ...
      try {
        ...
        console.log(`[StorageFactory] Migrated ${allSessions.length} sessions to ${newMode} mode`);
      } catch (error) {
