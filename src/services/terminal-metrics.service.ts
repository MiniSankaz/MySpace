...
export class TerminalMetricsService extends EventEmitter {
  ...
  private collectMetrics(): void {
    ...
    metrics.sessions = {
      ...
      avgResponseTime:
        this.sessionCounters.responseTimeCount > 0
          ? this.sessionCounters.responseTimeSum /
    ...
    metrics.network = {
      ...
      avgLatency:
        this.networkCounters.latencyCount > 0
    ...
    metrics.errors = {
      ...
      lastError:
        this.errorCounters.window.length > 0
  ...
  private checkAlerts(): void {
    ...
    if (this.metrics.cpu.percent > config.cpu) {
      alerts.push(
        `CPU usage high: ${this.metrics.cpu.percent.toFixed(1)}% > ${config.cpu}%`,
      );
    }
    ...
    if (this.metrics.sessions.total > config.sessions) {
      alerts.push(
        `Session count high: ${this.metrics.sessions.total} > ${config.sessions}`,
      );
    }
    ...
    if (this.metrics.errors.rate > config.errorRate) {
      alerts.push(
        `Error rate high: ${this.metrics.errors.rate}/min > ${config.errorRate}/min`,
      );
    }
  ...
  public recordError(type: string, error?: Error): void {
    ...
    if (this.errorCounters.window.length > this.errorCounters.maxWindowSize) {
      this.errorCounters.window = this.errorCounters.window.slice(
        -this.errorCounters.maxWindowSize,
      );
    }

    // Limit error types tracked
    if (
      Object.keys(this.errorCounters.types).length >=
      this.errorCounters.maxTypes
    ) {
      // Remove oldest type with lowest count
      const sortedTypes = Object.entries(this.errorCounters.types).sort(
        ([, a], [, b]) => a - b,
      );
  ...
  public getSummary(): any {
    ...
    return {
      ...
      averages: {
        ...
        responseTime:
          recent.reduce((sum, m) => sum + m.sessions.avgResponseTime, 0) /
          recent.length,
        errorRate:
          recent.reduce((sum, m) => sum + m.errors.rate, 0) / recent.length,
      },
      peaks: {
