...
export class ClaudeDirectService extends EventEmitter {
  ...
  async sendMessageWithSession(
    ...
    try {
      ...
      if (this.useBackgroundMode) {
        console.log(
          `[Claude Direct] Using session manager for session: ${sessionId}`,
        );
        ...
        const response = await this.sessionManager.sendMessageToSession(
          sessionId,
          fullMessage,
          userId,
        );
  ...
  async sendMessage(
    ...
    try {
      ...
      if (
        ...
        this.backgroundService.isReady()
      ) {
        ...
        try {
          const response = (await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error("Background service timeout")),
                90000,
              ),
            ),
          ])) as string;
          ...
          console.error(
            "[Claude Direct] Background service failed:",
            bgError.message,
          );
      ...
      if (
        response.includes("Invalid API key") ||
      ...
      console.log(
        "[Claude Direct] Response received, length:",
        response.length,
      );
      return { content: response };
    } catch (error: any) {
