...
export class MigrationService extends EventEmitter {
  ...
  private initialize(): void {
    ...
    // Log initial state
    console.log(
      "[MigrationService] Feature flags:",
      Object.fromEntries(this.featureFlags),
    );
  }
   ...
   */
  public getStatus(): MigrationStats & {
  ...
  private setupMonitoring(): void {
    ...
    setInterval(() => {
      ...
      if (newMode !== this.mode) {
        console.log(
          `[MigrationService] Mode changed from ${this.mode} to ${newMode}`,
        );
        this.mode = newMode;
        this.setupFeatureFlags();
      }
