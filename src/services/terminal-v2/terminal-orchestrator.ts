...
export class TerminalOrchestrator extends EventEmitter {
  ...
  constructor() {
    ...
    this.metricsCollector = new MetricsCollector(this.sessionManager, this.streamManager);
   ...
   */
  public async createTerminal(params: CreateTerminalParams): Promise<TerminalInfo> {
    if (!this.ready) {
   ...
   * Resize terminal
   */
  public resizeTerminal(sessionId: string, dimensions: { rows: number; cols: number }): void {
    // Update metadata
  ...
  public async resumeProject(projectId: string): Promise<TerminalInfo[]> {
    ...
    for (const session of sessions) {
      try {
        ...
        console.error(`[TerminalOrchestrator] Failed to recreate stream for session ${session.id}:`, error);
      }
    }
