...
export class StreamManager extends EventEmitter {
  ...
  private readonly RECONNECT_ATTEMPTS =
    terminalConfig.websocket.reconnectAttempts;
  ...
  public write(sessionId: string, data: string): void {
    ...
    if (stream.process) {
      ...
      stream.websocket &&
      stream.websocket.readyState === WebSocket.OPEN
    ) {
      // Write to WebSocket
  ...
  public async reconnectStream(sessionId: string): Promise<void> {
    ...
    while (attempts < this.RECONNECT_ATTEMPTS) {
      try {
        if (stream.type === StreamType.TERMINAL && !stream.process) {
          ...
          throw new Error(
            "Cannot recreate terminal process without session info",
          );
    ...
    throw new Error(
      `Failed to reconnect stream ${sessionId} after ${attempts} attempts`,
    );
  }
