...
export class SessionManager extends EventEmitter {
  ...
  private readonly SUSPENSION_TIMEOUT = terminalConfig.suspension.maxSuspensionTime;
  ...
  public updateSessionStatus(sessionId: string, status: SessionStatus): void {
    ...
    this.emit('session:status-changed', { session, oldStatus, newStatus: status });
  }

  /**
   * Update session metadata
   */
  public updateSessionMetadata(sessionId: string, metadata: Partial<SessionMetadata>): void {
    const session = this.sessions.get(sessionId);
    if (!session) {
      throw new Error(`Session ${sessionId} not found`);
    }
   ...
   */
  public updateSessionMetrics(sessionId: string, metrics: Partial<SessionMetrics>): void {
    const session = this.sessions.get(sessionId);
  ...
  private validateSessionCreation(projectId: string): void {
    ...
    if (this.sessions.size >= this.MAX_SESSIONS_TOTAL) {
      throw new Error(`Maximum total sessions (${this.MAX_SESSIONS_TOTAL}) reached`);
    }
    ...
    if (projectSessions && projectSessions.size >= this.MAX_SESSIONS_PER_PROJECT) {
      throw new Error(`Maximum sessions per project (${this.MAX_SESSIONS_PER_PROJECT}) reached`);
    }
  }
  ...
  private unfocusOldestSession(projectId: string): void {
    const sessions = this.listProjectSessions(projectId)
      ...
      .sort((a, b) => a.metrics.lastActivity.getTime() - b.metrics.lastActivity.getTime());
  ...
  private handleStatusTransition(session: TerminalSession, oldStatus: SessionStatus, newStatus: SessionStatus): void {
    // Handle state-specific logic
    if (newStatus === SessionStatus.ACTIVE && oldStatus !== SessionStatus.ACTIVE) {
      session.metrics.lastActivity = new Date();
    }
