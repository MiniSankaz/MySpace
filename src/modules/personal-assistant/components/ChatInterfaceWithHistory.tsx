...
export const ChatInterfaceWithHistory: React.FC<ChatInterfaceProps> = ({
  ...
  const formatMessage = (content: string) => {
    return content
      ...
      .replace(
        /```([\s\S]*?)```/g,
        '<pre class="p-3 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-x-auto my-2"><code>$1</code></pre>',
      )
      .replace(/\n/g, "<br />");
  };
  ...
  return (
    <div className="flex h-full bg-gray-50 dark:bg-gray-900 relative z-50">
      {/* Sidebar */}
      <div
        className={`${sidebarOpen ? "w-80" : "w-0"} transition-all duration-300 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden flex-shrink-0`}
      >
        {sidebarOpen && (
          <>
            {/* Sidebar Header */}
