...
export class AssistantService {
          ...
          [
            {
              role: "system",
              content:
                "You are a helpful AI assistant integrated into a personal assistant system. Help users with coding, technical questions, and general tasks. Respond in Thai if the user writes in Thai. You have access to the conversation history to maintain context.",
            },
            ...conversationHistory,
          ],
          userId,
  ...
  async sendDirectToClaude(
    ...
    try {
      ...
      const claudeResponse = await this.claudeService.sendMessageWithSession(
        sessionId,
        message,
        [
          {
            role: "system",
            content:
              "You are Claude, a helpful AI assistant. Respond naturally in the same language as the user. If they write in Thai, respond in Thai. If they write in English, respond in English. Be conversational and helpful. You have access to the conversation history to maintain context.",
          },
          ...conversationHistory,
        ],
        userId,
      ...
      const fallbackResponse: AssistantResponse = {
        message:
          "ขออภัย ไม่สามารถติดต่อ Claude ได้ในขณะนี้ กรุณาลองใหม่อีกครั้ง",
