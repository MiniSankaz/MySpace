...
export class ClaudeTerminal {
  ...
  private showHelp(): void {
    ...
    console.log(
      chalk.green("  status    ") + "- Show Claude connection status",
    );
  ...
  private showStatus(): void {
    console.log(chalk.yellow("\n📊 Status:"));
    console.log(
      `  Claude Process: ${this.claudeProcess ? chalk.green("Running") : chalk.red("Not Running")}`,
    );
    console.log(`  Terminal Active: ${chalk.green("Yes")}`);
    console.log(
      `  API Key: ${process.env.CLAUDE_API_KEY ? chalk.green("Set") : chalk.yellow("Not Set (using local mode)")}`,
    );
    console.log();
  }
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        if (mode === "code") {
          ...
          resolve(
            "This is an explanation of the concept you asked about. Claude would provide detailed information here.",
          );
        } else if (mode === "debug") {
          resolve(
            "Here's the debugging solution: Check your variables and ensure proper error handling.",
          );
        } else {
          resolve(
