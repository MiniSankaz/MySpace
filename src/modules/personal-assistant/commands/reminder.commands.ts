import { Command, AssistantContext, AssistantResponse, Reminder } from '../types';
...
export const reminderSetCommand: Command = {
  ...
  handler: async (context: AssistantContext, args: Record<string, any>): Promise<AssistantResponse> => {
    ...
    if (!title) {
      return {
        message: 'กรุณาระบุหัวข้อการเตือน ตัวอย่าง: `reminder set ประชุมทีม at 2pm`',
        suggestions: [
...
export const reminderListCommand: Command = {
  ...
  handler: async (context: AssistantContext): Promise<AssistantResponse> => {
    if (!context.userData || context.userData.reminders.length === 0) {
      return {
        message: 'คุณยังไม่มีการเตือนใดๆ ใช้ `reminder set [หัวข้อ] at [เวลา]` เพื่อเพิ่ม',
        suggestions: [
          'reminder set ประชุม at 2pm',
          'remind me อาหารเที่ยง in 30 minutes'
        ]
...
export const reminderDeleteCommand: Command = {
  ...
  handler: async (context: AssistantContext, args: Record<string, any>): Promise<AssistantResponse> => {
    ...
    const reminderIndex = context.userData.reminders.findIndex(r => 
      r.id === identifier || 
    ...
    const deletedReminder = context.userData.reminders.splice(reminderIndex, 1)[0];
