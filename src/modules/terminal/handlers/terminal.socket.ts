  ...
  terminalNamespace.on("connection", (socket: Socket) => {
    ...
    socket.on("terminal:create", async (data: TerminalData) => {
      try {
        ...
        socket.emit("terminal:error", {
          error: error.message || "Unknown error creating terminal session",
        });
      }
    });
    ...
    socket.on("terminal:resize", (data: TerminalData) => {
      try {
        ...
        if (!sessionId || !data.cols || !data.rows) {
          socket.emit("terminal:error", {
            error: "Session ID, cols, and rows required",
          });
          return;
        }
        ...
        const success = terminalService.resizeSession(
          sessionId,
          data.cols,
          data.rows,
        );
