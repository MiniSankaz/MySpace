...
class CircuitBreaker {
  ...
  recordFailure(): void {
    ...
    if (
      ...
      console.log(
        `[CircuitBreaker] Circuit opened after ${this.state.failures} failures. Next retry at ${this.state.nextRetryTime.toISOString()}`,
      );
    }
  }
    ...
    switch (this.state.state) {
      ...
      case "open":
        ...
        if (this.state.nextRetryTime && now >= this.state.nextRetryTime) {
          ...
          console.log(
            "[CircuitBreaker] Circuit entering half-open state for testing",
          );
          return true;
        }
        return false;
...
export class TerminalWebSocketMultiplexer extends EventEmitter {
  ...
  private initializePrimaryConnection(): void {
    console.log(
      "Terminal WebSocket Multiplexer initialized for standalone servers",
    );
  ...
  private setupPrimarySocketListeners(): void {
    ...
    console.log(
      "Using standalone WebSocket servers - no primary socket needed",
    );
  ...
  async connectSession(
    ...
    if (this.connections.has(sessionId)) {
      ...
      if (
        ...
        console.log(
          `[${sessionId}] Cleaning up old connection and resetting circuit breaker`,
        );
    ...
    if (!SessionValidator.isValidSessionId(sessionId)) {
      console.error(
        `Invalid session ID format: ${sessionId}. Expected format: session_{timestamp}_{random}`,
      );
  ...
  private async createWebSocketConnection(
    ...
    if (!connection.circuitBreaker.shouldAllow()) {
      ...
      console.warn(
        `[${sessionId}] Circuit breaker is OPEN. Not attempting connection. Next retry: ${cbState.nextRetryTime}`,
      );
    ...
    const token =
      this.options.auth?.token || localStorage.getItem("accessToken");
    const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";

    // Determine the correct port based on terminal type
    const port =
      type === "system"
        ? "terminalConfig.websocket.port"
        : "terminalConfig.websocket.claudePort";
    ...
    websocket.onopen = () => {
      console.log(
        `${type} terminal WebSocket connected for session ${sessionId}`,
      );
      ...
      try {
        ...
        if (message.type === "stream" || message.type === "data") {
          if (connection.mode === "active") {
            ...
            this.emit("session:background-activity", {
              sessionId,
              data: message.data,
            });
          }
    ...
    websocket.onclose = (event) => {
      console.log(
        `${type} WebSocket closed for session ${sessionId}:`,
        event.code,
        event.reason,
      );
      ...
      const isCircuitBreakerClose =
        event.code >= process.env.PORT || (4000 && event.code <= 4099);
      ...
      if (
        ...
        if (event.code === 1005) {
          ...
          if (!connection.circuitBreaker.shouldAllow()) {
            const cbState = connection.circuitBreaker.getState();
            console.warn(
              `[${sessionId}] Circuit breaker open after ${cbState.failures} failures. Stopping reconnection.`,
            );
            this.emit("session:circuit-open", { sessionId, state: cbState });
          ...
          if (connection.reconnectAttempts >= 2) {
            console.warn(
              `Session ${sessionId} repeatedly failing with code 1005, stopping reconnection`,
            );
        ...
        if (isCircuitBreakerClose) {
          console.log(
            `Session ${sessionId} closed by circuit breaker (code ${event.code}), not reconnecting`,
          );
        } else {
          console.log(
            `Session ${sessionId} closed cleanly or during shutdown, not reconnecting`,
          );
        }
      }
    };
  ...
  setSessionMode(sessionId: string, mode: "active" | "background"): void {
    ...
    if (
      oldMode === "background" &&
      mode === "active" &&
      connection.outputBuffer.length > 0
    ) {
      console.log(
        `[${sessionId}] Flushing ${connection.outputBuffer.length} buffered entries`,
      );
   ...
   */
  private handleMultiplexedMessage(
    sessionId: string,
    type: string,
    payload: any,
  ): void {
  ...
  private scheduleReconnect(sessionId: string): void {
    ...
    if (!connection.circuitBreaker.shouldAllow()) {
      const cbState = connection.circuitBreaker.getState();
      console.warn(
        `[${sessionId}] Circuit breaker preventing reconnection. State: ${cbState.state}`,
      );
      this.emit("session:circuit-open", { sessionId, state: cbState });
    ...
    this.updateConnectionStatus(sessionId, "connecting");
    this.emit("session:reconnecting", {
      sessionId,
      attempt: connection.reconnectAttempts,
    });

    // Use circuit breaker's backoff delay calculation
    const delay = connection.circuitBreaker.getBackoffDelay(
      connection.reconnectAttempts,
    );
    console.log(
      `Scheduling reconnect for session ${sessionId} in ${delay}ms (attempt ${connection.reconnectAttempts})`,
    );
    ...
    const timer = setTimeout(async () => {
      ...
      if (this.isShuttingDown) {
        console.log(
          `Cancelling reconnect for session ${sessionId} - shutting down`,
        );
        return;
      }
  ...
  disconnectSession(sessionId: string): void {
    ...
    console.log(
      `Disconnecting UI for session ${sessionId} (keeping process alive)`,
    );
...
export function createTerminalMultiplexer(
