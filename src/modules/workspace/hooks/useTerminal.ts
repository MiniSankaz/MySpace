...
export const useTerminal = ({ sessionId, onOutput, onExit, onError }: UseTerminalOptions) => {
  ...
  useEffect(() => {
    ...
    socket.on('exit', ({ sessionId: sid, exitCode }: { sessionId: string; exitCode: number }) => {
      if (sid === sessionId) {
        onExit?.(exitCode);
      }
    });
  ...
  const createSession = useCallback(async (projectId: string, type: 'system' | 'claude', tabName: string) => {
    try {
  ...
  // Get command history
  const getHistory = useCallback(async (limit = 100) => {
    try {
      const response = await fetch(`/api/workspace/terminal/session/${sessionId}/history?limit=${limit}`);
