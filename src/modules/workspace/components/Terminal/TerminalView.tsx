...
const TerminalView: React.FC<TerminalViewProps> = ({
    ...
    setOutput((prev) => {
      ...
      lines.forEach((line) => {
        if (line || lines.length === 1) {
          // Keep empty lines except trailing
          newOutput.push(line);
        }
      });
  ...
  const isConnected =
    type === "system" ? systemWs.isConnected : claudeWs.isConnected;
  const isConnecting =
    type === "system" ? systemWs.isConnecting : claudeWs.isConnecting;
  const sendInput = type === "system" ? systemWs.sendInput : claudeWs.sendInput;
  const sendControl =
    type === "system" ? systemWs.sendControl : claudeWs.sendControl;
  const connect = type === "system" ? systemWs.connect : claudeWs.connect;
  const disconnect =
    type === "system" ? systemWs.disconnect : claudeWs.disconnect;
