...
const TerminalContainerV4: React.FC<TerminalContainerV4Props> = ({ project }) => {
  ...
  const loadSessions = useCallback(async (projectId: string) => {
    try {
      ...
      if (data.migrationInfo) {
        ...
        console.log(`[Terminal V4] Using ${data.migrationInfo.usingNewSystem ? 'NEW' : 'LEGACY'} system`);
      }
      
      setSessions(data.sessions || []);
      console.log(`[Terminal V4] Loaded ${data.sessions?.length || 0} sessions`);
  ...
  // Create new session
  const createSession = useCallback(async (mode: 'normal' | 'claude' = 'normal') => {
    try {
      setLoading(true);
      setError(null);
      
      console.log(`[Terminal V4] Creating ${mode} session for project: ${project.id}`);
  ...
  useEffect(() => {
    if (project.id !== previousProjectIdRef.current) {
      console.log(`[Terminal V4] Project changed: ${previousProjectIdRef.current} -> ${project.id}`);
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-800/50 bg-gray-900/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={loading || sessions.length >= LAYOUTS[currentLayout].rows * LAYOUTS[currentLayout].cols}
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-3 py-2 bg-gray-800/50 border-b border-gray-700/50">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">{session.tabName}</span>
                    </div>
                    <button
