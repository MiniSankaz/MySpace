...
const XTermViewV3: React.FC<XTermViewV3Props> = ({
  ...
  const getWebSocketUrl = useCallback(() => {
    const useNewEndpoint =
      process.env.NEXT_PUBLIC_USE_NEW_TERMINAL_API === "true" || true;
  ...
  useEffect(() => {
    ...
    if (xtermRef.current) {
      console.log(
        `[XTermViewV3] Terminal already initialized for ${sessionId}`,
      );
      return;
    }
      ...
      ws.onmessage = (event) => {
        try {
          ...
          switch (message.type) {
            case "connected":
              ...
              if (xtermRef.current) {
                xtermRef.current.writeln(
                  `📡 System: ${message.system || "legacy"} | Mode: ${message.migrationMode || "unknown"}`,
                );
              }
              break;
            ...
            default:
              console.log(
                `[XTermViewV3] Unknown message type: ${message.type}`,
              );
          }
        } catch (error) {
  ...
  return (
    <div className="relative h-full w-full bg-black">
      ...
      >
        ...
        {systemType && <span className="opacity-75">({systemType})</span>}
      </div>
