    ...
    <AnimatePresence>
      {showHelp && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className={`px-6 py-4 border-b ${theme === 'dark' ? 'border-gray-700 bg-gray-900/50' : 'border-gray-200 bg-gray-50'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {shortcuts.map((section) => (
                  <div key={section.category}>
                    <h3 className={`text-sm font-semibold uppercase tracking-wider mb-3 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                      {section.category}
                    </h3>
                          ...
                          <div className="flex items-center space-x-1">
                            {shortcut.keys.map((key, i) => (
                              <React.Fragment key={i}>
                                {i > 0 && <span className={`text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>+</span>}
                                <kbd className={`px-2 py-1 text-xs font-medium ${theme === 'dark' ? 'bg-gray-700 text-gray-300 border-gray-600' : 'bg-gray-100 text-gray-700 border-gray-300'} border rounded`}>
                                  {key}
                                </kbd>
                              </React.Fragment>
                            ))}
            ...
            {/* Footer */}
            <div className={`px-6 py-3 border-t ${theme === 'dark' ? 'border-gray-700 bg-gray-900/50' : 'border-gray-200 bg-gray-50'}`}>
              <div className="flex items-center justify-between">
                <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                  Press <kbd className={`px-1.5 py-0.5 mx-1 ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-200'} rounded text-xs`}>⌘⇧?</kbd> to toggle this help
                </p>
                <button
                  onClick={() => setShowHelp(false)}
