...
async function testSystemTerminal() {
  ...
  return new Promise((resolve) => {
    ...
    ws.on('open', () => {
      ...
      // Send test command
      ws.send(JSON.stringify({ type: 'input', data: 'echo "Smoke test successful"\r' }));
    });
    ...
    setTimeout(() => {
      ...
      if (connected && commandExecuted) {
        ...
        if (!commandExecuted) testResults.failed.push('SystemTerminal-Commands');
      }
...
async function testClaudeTerminal() {
  ...
  return new Promise((resolve) => {
    ...
    ws.on('message', (data) => {
      const msg = JSON.parse(data);
      if (msg.type === 'stream' && (msg.data.includes('Claude') || msg.data.includes('Welcome'))) {
        claudeStarted = true;
    ...
    setTimeout(() => {
      ...
      if (claudeStarted) {
        ...
        console.log('  ⚠️ Claude CLI did not fully initialize (may need more time)');
      }
...
async function testCircuitBreaker() {
  ...
  if (reconnectAttempts < maxAttempts) {
    console.log(`  ✅ Circuit breaker activated after ${reconnectAttempts} attempts`);
...
async function testSessionPersistence() {
  ...
  await new Promise((resolve) => {
    ws1.on('open', () => {
      ws1.send(JSON.stringify({ type: 'input', data: 'TEST_VAR="persistent"\r' }));
      setTimeout(() => {
        ws1.close();
        resolve();
      }, 1000);
  ...
  if (sessionPersisted) {
    ...
    console.log('  ⚠️ Session state not fully preserved (expected in some configurations)');
  }
}
