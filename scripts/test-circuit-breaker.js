...
function connectWebSocket() {
  ...
  const ws = new WebSocket(`${WS_URL}?sessionId=${TEST_SESSION_ID}&projectId=${PROJECT_ID}`);
  ...
  ws.on('close', (code, reason) => {
    lastCloseCode = code;
    console.log(`[Attempt ${reconnectCount}] ❌ Closed with code ${code}: ${reason}`);
...
function checkTestResult() {
  ...
  if (testPassed) {
    console.log('✅ TEST PASSED: Circuit breaker successfully prevented infinite loop');
    process.exit(0);
  } else if (reconnectCount <= 5) {
    console.log('✅ TEST PASSED: Reconnections stopped within reasonable limit');
    process.exit(0);
  } else {
