  ...
  try {
    ...
    for (const testProject of TEST_PROJECTS) {
      try {
        ...
        if (!owner) {
          console.log(
            `   ⚠️ Skipping project ${testProject.name} - owner not found`,
          );
          continue;
        }
        ...
        console.log(
          `   ✅ Created project: ${project.name} (owner: ${owner.username})`,
        );
      } catch (error) {
        console.error(
          `   ❌ Failed to create project ${testProject.name}:`,
          error,
        );
      }
    }
    ...
    for (const project of TEST_PROJECTS) {
      try {
        ...
        console.log(
          `   ✅ Created terminal session for project: ${project.name}`,
        );
      } catch (error) {
        console.error(
          `   ⚠️ Failed to create terminal session for ${project.name}:`,
          error,
        );
      }
    }
