...
async function cleanupAndSetupSankaz() {
  ...
  try {
    ...
    let sankazUser = await prisma.user.findFirst({
      where: {
        OR: [{ username: "sankaz" }, { email: { contains: "sankaz" } }],
      },
    });
    ...
    if (conversations.length > 0) {
      ...
      for (const conversation of conversations) {
        try {
          ...
          console.warn(
            `⚠️  Failed to migrate conversation ${conversation.id}: ${(error as Error).message}`,
          );
          // Continue with other conversations
        }
      }
      console.log(
        `✅ Migrated ${migratedCount}/${conversations.length} conversations to sankaz\n`,
      );
    } else {
