...
pre_deployment_checks() {
    ...
    if curl -s http://localhost:$PORT_GATEWAY_MAIN/api/health >/dev/null 2>&1; then
...
run_smoke_tests() {
    ...
    local staging_url="http://localhost:$PORT_SERVICE_USER"
    local test_count=0
    local pass_count=0
...
run_validation_tests() {
    ...
    for i in {1..10}; do
        if ! curl -s "http://localhost:$PORT_SERVICE_USER/api/health" >/dev/null; then
...
const STAGING_URL = 'http://localhost:$PORT_SERVICE_USER';
...
EOF
    ...
    echo -e "• Staging URL: ${BLUE}http://localhost:$PORT_SERVICE_USER${NC}"
