...
class TerminalStorageMigration {
  ...
  private displayConfig(): void {
    ...
    console.log(chalk.gray('  Dry Run:'), this.options.dryRun ? chalk.yellow('Yes') : chalk.green('No'));
    console.log(chalk.gray('  Force:'), this.options.force ? chalk.yellow('Yes') : chalk.green('No'));
    console.log(chalk.gray('  Verbose:'), this.options.verbose ? chalk.green('Yes') : chalk.gray('No'));
  }
  ...
  private async validateEnvironment(): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.gray(`  Memory usage: ${Math.round(info.memoryUsage / 1024 / 1024)}MB`));
  ...
  private async confirmMigration(): Promise<boolean> {
    ...
    console.log(chalk.yellow('กรุณายืนยันด้วยการรัน command พร้อม --force flag'));
    return false;
  }
  ...
  private displayResults(): void {
    ...
    console.log(chalk.white('  Failed:'), this.stats.failed > 0 ? chalk.red(this.stats.failed) : chalk.gray(this.stats.failed));
    console.log(chalk.white('  Skipped:'), this.stats.skipped > 0 ? chalk.yellow(this.stats.skipped) : chalk.gray(this.stats.skipped));
    ...
    console.log(chalk.white('  Success Rate:'), 
      successRate >= 90 ? chalk.green(`${successRate}%`) :
      successRate >= 70 ? chalk.yellow(`${successRate}%`) :
  ...
  private async verifyMigration(): Promise<void> {
    ...
    try {
      ...
      if (storageInfo.sessionCount < this.stats.migrated) {
        ...
        console.log(chalk.yellow(`  Expected: ${this.stats.migrated}, Found: ${storageInfo.sessionCount}`));
      } else {
      ...
      if (this.stats.migrated > 0) {
        ...
        if (legacySessions.length > 0) {
          ...
          const migratedSession = await this.storageService.getSession(testSession.id);
          
          if (migratedSession) {
            console.log(chalk.green('  ✓ Sample session verified'));
          } else {
            console.log(chalk.red('  ✗ Sample session not found in new storage'));
          }
        }
      }
...
program
  ...
  .action(async (options) => {
    try {
      ...
      // Run migration
      const migration = new TerminalStorageMigration(options as MigrationOptions);
      await migration.run();
