...
async function testSettings() {
  ...
  try {
    ...
    await settingsService.setUserConfig(testUserId, 'theme', 'dark', 'appearance');
    await settingsService.setUserConfig(testUserId, 'language', 'th', 'preferences');
    await settingsService.setUserConfig(testUserId, 'timezone', 'Asia/Bangkok', 'preferences');
    ...
    await settingsService.setApiConfig(testUserId, 'maxRequestsPerHour', 1000, 'limits', 'Max requests per hour');
    await settingsService.setApiConfig(testUserId, 'webhookUrl', 'https://example.com/webhook', 'webhooks', 'Webhook endpoint');
    ...
    await settingsService.setSystemConfig('maintenanceMode', false, 'system', 'Maintenance mode flag');
    await settingsService.setSystemConfig('maxUploadSize', '10MB', 'storage', 'Maximum upload size');
    ...
    const notificationConfigs = await settingsService.getUserConfig(testUserId, null, 'notifications');
    console.log('✅ Bulk configs saved:', notificationConfigs.length, 'notification settings');
    
    // Test Default Settings
    console.log('\n6️⃣ Testing Default Settings...');
    const defaultUser = settingsService.getDefaultUserSettings();
    console.log('✅ Default user settings:', Object.keys(defaultUser).length, 'categories');
    
    const defaultApi = settingsService.getDefaultApiSettings();
    console.log('✅ Default API settings:', Object.keys(defaultApi).length, 'categories');
    
    const defaultSystem = settingsService.getDefaultSystemSettings();
    console.log('✅ Default system settings:', Object.keys(defaultSystem).length, 'categories');
