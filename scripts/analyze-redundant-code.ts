...
// Backup files
const backupFiles = [
  'src/backup/terminal-v1',
  'src/backup/terminal-backup'
];
...
function main() {
  ...
  for (const group of redundantServices) {
    for (const file of group.files) {
      ...
      if (analysis) {
        ...
        console.log(`  ${chalk.red('✗')} ${file} (${formatSize(analysis.size)}) - ${group.reason}`);
      }
    }
  }
  ...
  for (const file of oldComponents) {
    const analysis = analyzeFile(file);
    if (analysis) {
      analyses.push({ ...analysis, category: 'duplicate', reason: 'Replaced by V3/V4' });
      console.log(`  ${chalk.red('✗')} ${file} (${formatSize(analysis.size)}) - Replaced by newer version`);
    }
  }
  ...
  for (const file of backupFiles) {
    ...
    if (analysis) {
      ...
      console.log(`  ${chalk.red('✗')} ${file} (${formatSize(analysis.size)}) - Backup directory`);
    }
  }
  ...
  console.log(`Can delete: ${deletableFiles.length} files (${formatSize(deletableSize)})`);
  
  // Generate cleanup script
  if (deletableFiles.length > 0) {
    const scriptPath = path.join(process.cwd(), 'scripts', 'cleanup-redundant-code.sh');
    const script = [
      '#!/bin/bash',
      '',
    ...
    console.log(`  Run: ${chalk.yellow('./scripts/cleanup-redundant-code.sh')}`);
  }
  ...
  console.log('4. Consider keeping backups for 1-2 weeks before permanent deletion');
}
