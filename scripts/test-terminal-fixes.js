...
async function authenticate() {
  try {
    ...
    try {
      const response2 = await axios.post(`${CONFIG.BASE_URL}/api/ums/auth/login`, {
...
async function testDatabaseForeignKeyFix() {
  ...
  try {
    ...
    if (response.data && response.data.id) {
      log(`Session created successfully with auto-project creation: ${response.data.id}`, 'success');
      RESULTS.passed.push({ test: testName, message: 'Auto-project creation working' });
      return true;
    } else {
      throw new Error('No session ID returned');
    }
  } catch (error) {
    log(`Failed to create session: ${error.response?.data?.error || error.message}`, 'error');
...
async function testClaudeCLIIntegration() {
  ...
  return new Promise((resolve) => {
    ...
    const ws = new WebSocket(`ws://127.0.0.1:${CONFIG.CLAUDE_TERMINAL_PORT}/?projectId=${CONFIG.TEST_PROJECT_ID}&sessionId=${sessionId}`);
    ...
    const timeout = setTimeout(() => {
      ...
      if (!claudeReady) {
        RESULTS.warnings.push({ 
          test: testName, 
          message: 'Claude CLI did not report ready within timeout, but connection was established' 
        });
      } else {
        RESULTS.passed.push({ 
          test: testName, 
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'claude_ready') {
          ...
          const readyIndicators = ['claude>', 'Claude>', 'Welcome to Claude', 'How can I help'];
          if (readyIndicators.some(indicator => message.data.toLowerCase().includes(indicator.toLowerCase()))) {
            claudeReady = true;
...
async function testSessionPersistence() {
  ...
  try {
    ...
    log(`Created ${succeeded}/3 sessions successfully`, succeeded === 3 ? 'success' : 'warning');
...
async function testGracefulDegradation() {
  ...
  return new Promise((resolve) => {
    const ws = new WebSocket(`ws://127.0.0.1:${CONFIG.SYSTEM_TERMINAL_PORT}/?projectId=${longProjectId}`);
    ...
    ws.on('error', (error) => {
      clearTimeout(timeout);
      log(`WebSocket error (expected in degradation test): ${error.message}`, 'warning');
      RESULTS.warnings.push({ 
        test: testName, 
...
async function testCommandExecution() {
  ...
  return new Promise((resolve) => {
    const ws = new WebSocket(`ws://127.0.0.1:${CONFIG.SYSTEM_TERMINAL_PORT}/?projectId=${CONFIG.TEST_PROJECT_ID}`);
...
async function runTests() {
  ...
  try {
    ...
    if (error.response && error.response.status === 503) {
      log('Development server is running but database is unavailable (testing graceful degradation)', 'warning');
    } else if (error.code === 'ECONNREFUSED') {
      log('Development server is not running. Please run: npm run dev', 'error');
      process.exit(1);
    } else {
