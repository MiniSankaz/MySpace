...
class LoadTester {
  ...
  async run(): Promise<TestResult> {
    ...
    console.log(`Total sessions: ${this.config.numProjects * this.config.sessionsPerProject}`);
  ...
  private async createSession(sessionId: string, projectId: string): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        fetch(`${this.config.baseUrl}/api/terminal-v2/create`, {
          ...
          ws.on('error', (error) => {
            this.metrics.errors.push(`WS error for ${sessionId}: ${error.message}`);
            reject(error);
          });
  ...
  private calculateResults(): TestResult {
    ...
    return {
      ...
      failedConnections: (this.config.numProjects * this.config.sessionsPerProject) - this.connections.size,
...
async function main() {
  ...
  console.log(`Successful Connections: ${chalk.green(results.successfulConnections)}`);
  ...
  const connectionRate = (results.successfulConnections / results.totalSessions) * 100;
