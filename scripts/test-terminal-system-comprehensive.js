...
async function createTerminalSession(type, tabName = `Test-${type}-Terminal`) {
  try {
    ...
    throw new Error(`Failed to create ${type} terminal: ${error.response?.data?.error || error.message}`);
  }
}
...
async function testSystemTerminalConnection() {
  return new Promise(async (resolve, reject) => {
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'connected') {
          ...
          TEST_RESULTS.systemTerminal.tests.push({ name: testName, status: 'PASS', message: 'Connected successfully' });
          resolve(true);
        }
      } catch (error) {
    ...
    ws.on('error', (error) => {
      ...
      TEST_RESULTS.systemTerminal.tests.push({ name: testName, status: 'FAIL', message: error.message });
      reject(error);
    });
  });
}
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'connected') {
          ...
          ws.send(JSON.stringify({ type: 'input', data: commands[commandIndex] + '\r' }));
        } else if (message.type === 'stream') {
          commandResults.push(message.data);
          console.log('📥 Command output received:', message.data.substring(0, 50));
          
          // Check if we see a prompt indicating command completed
          if (message.data.includes('%') || message.data.includes('$') || message.data.includes('#')) {
            // Command likely completed, send next one if available
            if (commandIndex < commands.length - 1) {
              setTimeout(() => {
                commandIndex++;
                if (commands[commandIndex]) { // Safety check
                  console.log(`📤 Sending command: ${commands[commandIndex]}`);
                  ws.send(JSON.stringify({ type: 'input', data: commands[commandIndex] + '\r' }));
                }
              }, 1000);
          ...
          if (commandIndex >= commands.length - 1 && message.data.includes('Hello Terminal Test')) {
...
async function testSystemTerminalReconnection() {
  return new Promise(async (resolve, reject) => {
    ...
    ws1.on('message', (data) => {
      try {
        ...
        if (message.type === 'connected' && firstConnection) {
          ...
          ws1.send(JSON.stringify({ type: 'input', data: 'echo "First connection test"\r' }));
          firstConnection = false;
        } else if (message.type === 'stream' && message.data.includes('First connection test')) {
          console.log('📥 First connection command executed, closing connection');
          ws1.close();
          ...
          setTimeout(() => {
            ...
            ws2.on('message', (data) => {
              try {
                ...
                if (message.type === 'reconnected' || message.type === 'connected') {
                  console.log('📥 Reconnection successful, checking history');
                  
                  // Send another command to verify session persistence
                  setTimeout(() => {
                    ws2.send(JSON.stringify({ type: 'input', data: 'echo "Reconnection test"\r' }));
                  }, 1000);
                } else if (message.type === 'stream' && message.data.includes('Reconnection test')) {
                  clearTimeout(timeout);
                  ws2.close();
                  TEST_RESULTS.systemTerminal.passed++;
                  TEST_RESULTS.systemTerminal.tests.push({ 
            ...
            ws2.on('error', (error) => {
              clearTimeout(timeout);
              TEST_RESULTS.systemTerminal.failed++;
              TEST_RESULTS.systemTerminal.tests.push({ name: testName, status: 'FAIL', message: `Reconnection failed: ${error.message}` });
              reject(error);
            });
          }, 2000);
        }
    ...
    ws1.on('error', (error) => {
      clearTimeout(timeout);
      TEST_RESULTS.systemTerminal.failed++;
      TEST_RESULTS.systemTerminal.tests.push({ name: testName, status: 'FAIL', message: `First connection failed: ${error.message}` });
      reject(error);
    });
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'connected') {
          console.log('✅ Claude terminal connected, waiting for Claude CLI startup');
        } else if (message.type === 'stream' && message.data.includes('Claude>')) {
          clearTimeout(timeout);
          ws.close();
    ...
    ws.on('error', (error) => {
      ...
      TEST_RESULTS.claudeTerminal.tests.push({ name: testName, status: 'FAIL', message: error.message });
      reject(error);
    });
  });
}
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'stream') {
          ...
          } else if (claudeReady && (message.data.includes('Available commands') || message.data.includes('help'))) {
...
async function testKnowledgeBaseIntegration() {
  ...
  try {
    ...
    if (kmFilesExist) {
      ...
      const claudeTerminalContent = fs.readFileSync('/Users/sem4pro/Stock/port/src/server/websocket/claude-terminal-ws.js', 'utf8');
      const hasKMIntegration = claudeTerminalContent.includes('terminalKBIntegration') || 
    ...
    TEST_RESULTS.kmIntegration.failed++;
    TEST_RESULTS.kmIntegration.tests.push({ name: testName, status: 'FAIL', message: error.message });
    return false;
  }
}
...
async function testMultipleTerminalsSynchronously() {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < 3; i++) {
      // System terminals
      const systemWs = createWebSocket(CONFIG.SYSTEM_TERMINAL_PORT, `multi_system_${i}_${Date.now()}`);
      ...
      systemWs.on('message', (data) => {
        try {
          ...
          if (message.type === 'connected') {
            ...
            // Send unique command
            setTimeout(() => {
              systemWs.send(JSON.stringify({ type: 'input', data: `echo "System terminal ${i} test"\r` }));
            }, 1000 * i); // Stagger commands
          } else if (message.type === 'stream' && message.data.includes(`System terminal ${i} test`)) {
            terminalResults[systemId].commandExecuted = true;
            completedTerminals++;
            console.log(`📥 System terminal ${i} command completed (${completedTerminals}/${totalTerminals})`);
      ...
      // Claude terminals
      const claudeWs = createWebSocket(CONFIG.CLAUDE_TERMINAL_PORT, `multi_claude_${i}_${Date.now()}`);
        ...
        try {
          ...
          if (message.type === 'connected') {
            ...
            if (message.data.includes('Claude>') && !claudeReady) {
              ...
              // Send help command
              setTimeout(() => {
                claudeWs.send(JSON.stringify({ type: 'input', data: 'help\r' }));
              }, 2000 * i); // Stagger commands
            } else if (claudeReady && message.data.includes('Available commands')) {
              terminalResults[claudeId].commandExecuted = true;
              completedTerminals++;
              console.log(`📥 Claude terminal ${i} command completed (${completedTerminals}/${totalTerminals})`);
    ...
    function checkResults() {
      ...
      const allConnected = Object.values(terminalResults).every(result => result.connected);
      const allExecuted = Object.values(terminalResults).every(result => result.commandExecuted);
...
async function testTerminalSessionPersistence() {
  ...
  try {
    ...
    const newSession = await createTerminalSession('system', 'Persistence-Test');
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (message.type === 'stream' && message.data.includes('Loaded environment')) {
          ...
          setTimeout(() => {
            ...
            ws.send(JSON.stringify({ type: 'input', data: 'echo $NODE_ENV\r' }));
          }, 3000);
        } else if (message.type === 'stream' && message.data.includes('development')) {
...
async function checkWebSocketServerStatus() {
  ...
  try {
    const systemServerRunning = await checkPort(CONFIG.SYSTEM_TERMINAL_PORT, 'system');
    const claudeServerRunning = await checkPort(CONFIG.CLAUDE_TERMINAL_PORT, 'claude');
...
function generateReport() {
  ...
  const totalTests = Object.values(TEST_RESULTS).reduce((sum, category) => sum + category.passed + category.failed, 0);
  const totalPassed = Object.values(TEST_RESULTS).reduce((sum, category) => sum + category.passed, 0);
  const totalFailed = Object.values(TEST_RESULTS).reduce((sum, category) => sum + category.failed, 0);
  ...
  if (totalFailed === 0) {
    console.log('   🎉 All tests passed! Terminal system is functioning correctly.');
  } else {
    if (TEST_RESULTS.systemTerminal.failed > 0) {
      console.log('   🔧 System Terminal Issues: Check WebSocket server on port 4001');
    }
    if (TEST_RESULTS.claudeTerminal.failed > 0) {
      console.log('   🤖 Claude Terminal Issues: Verify Claude CLI is installed and WebSocket server on port 4002');
    }
    if (TEST_RESULTS.kmIntegration.failed > 0) {
      console.log('   📚 KM Integration Issues: Complete Knowledge Base module implementation');
    }
    if (TEST_RESULTS.multiTerminal.failed > 0) {
      console.log('   🔄 Multi-Terminal Issues: Check session management and parallel processing');
    }
  }
...
async function runAllTests() {
  ...
  try {
    ...
    try {
      ...
      console.error('❌ System terminal connection test failed:', error.message);
    }
    ...
    try {
      ...
      console.error('❌ System terminal reconnection test failed:', error.message);
    }
    ...
    try {
      ...
      console.error('❌ Terminal session persistence test failed:', error.message);
    }
    ...
    try {
      ...
      console.error('❌ Claude terminal connection test failed:', error.message);
    }
    
    try {
      await testClaudeTerminalAICommands();
    } catch (error) {
      console.error('❌ Claude terminal AI commands test failed:', error.message);
    }
    ...
    try {
      ...
      console.error('❌ Knowledge base integration test failed:', error.message);
    }
