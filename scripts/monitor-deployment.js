...
function loadMetrics() {
  try {
    if (fs.existsSync(MONITOR_CONFIG.metricsFile)) {
      const data = JSON.parse(fs.readFileSync(MONITOR_CONFIG.metricsFile, 'utf8'));
      Object.assign(metrics, data);
      console.log('📊 Loaded existing metrics from previous monitoring session');
    }
  } catch (error) {
...
function saveMetrics() {
  fs.writeFileSync(MONITOR_CONFIG.metricsFile, JSON.stringify(metrics, null, 2));
}
...
function generateAlert(severity, message, details = {}) {
  ...
  console.log(`${colors[severity]}⚠️ [${severity.toUpperCase()}] ${message}${reset}`);
  
  // Save alerts
  fs.writeFileSync(MONITOR_CONFIG.alertsFile, JSON.stringify(metrics.alerts, null, 2));
...
function checkMemoryUsage() {
  ...
  if (heapUsedMB > 100) {
    generateAlert('warning', `High memory usage: ${heapUsedMB.toFixed(2)}MB`, { heapUsedMB });
  }
...
function calculateStats() {
  const totalConnections = metrics.connectionSuccesses + metrics.connectionFailures;
  const successRate = totalConnections > 0 
    ? (metrics.connectionSuccesses / totalConnections * 100).toFixed(2) 
    : 0;
  
  const avgLatency = metrics.averageLatency.length > 0
    ? (metrics.averageLatency.reduce((a, b) => a + b, 0) / metrics.averageLatency.length).toFixed(2)
    : 0;
...
async function runMonitoringCheck() {
  ...
  const checkResult = {
    timestamp: new Date().toISOString(),
    systemTerminal: await checkWebSocketHealth(MONITOR_CONFIG.systemPort, 'System Terminal'),
    claudeTerminal: await checkWebSocketHealth(MONITOR_CONFIG.claudePort, 'Claude Terminal'),
  ...
  if (checkResult.systemTerminal.status === 'unhealthy') {
    generateAlert('critical', 'System Terminal is not responding', checkResult.systemTerminal);
  }
  
  if (checkResult.claudeTerminal.status === 'unhealthy') {
    generateAlert('warning', 'Claude Terminal is not responding', checkResult.claudeTerminal);
  }
  
  if (!checkResult.circuitBreaker) {
    generateAlert('warning', 'Circuit breaker may not be functioning correctly');
  }
  ...
  if (metrics.checks.length % 60 === 0) { // Every hour
...
function generateHourlyReport() {
  ...
  if (parseFloat(stats.successRate) < 95) {
    generateAlert('warning', `Success rate below 95%: ${stats.successRate}%`, stats);
  }
}
...
startMonitoring().catch(error => {
  ...
  generateAlert('critical', 'Monitoring system crashed', { error: error.message });
  process.exit(1);
});