...
async function testAPI() {
  ...
  try {
    ...
    const listResponse = await axios.get(`${API_URL}/list?projectId=${testProjectId}`);
    ...
    log(`  Buffered output: ${focusResponse.data.bufferedOutput ? 'yes' : 'no'}`);
    ...
    log(`  Previous focus: ${switchFocusResponse.data.previousFocus || 'none'}`);
    ...
    const cleanupResponse = await axios.delete(`${API_URL}/cleanup?projectId=${testProjectId}`);
