...
async function testAssistantLogging(auth) {
  ...
  // Get assistant logs
  const logsResponse = await fetch(`${BASE_URL}/api/logs/assistant?projectId=${TEST_PROJECT_ID}`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${auth.token}`,
      'Cookie': auth.cookies || '',
...
async function testTerminalLogging(auth) {
  ...
  const logsResponse = await fetch(`${BASE_URL}/api/logs/terminal?projectId=${TEST_PROJECT_ID}`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${auth.token}`,
      'Cookie': auth.cookies || '',
  ...
  if (!logsResponse.ok) {
    log(`⚠️ Terminal logs not available: ${logsResponse.statusText}`, colors.yellow);
    return null;
  }

  const logsData = await logsResponse.json();
  log(`✅ Terminal logs retrieved:`, colors.green);
  log(`   - Total sessions: ${logsData.stats?.totalSessions || 0}`, colors.white);
  log(`   - Active sessions: ${logsData.stats?.activeSessions || 0}`, colors.white);
  log(`   - Total commands: ${logsData.stats?.totalCommands || 0}`, colors.white);
  log(`   - Error rate: ${logsData.stats?.errorRate?.toFixed(2) || 0}%`, colors.white);
...
async function testLogsSummary(auth) {
  ...
  const summaryResponse = await fetch(`${BASE_URL}/api/logs/summary?projectId=${TEST_PROJECT_ID}`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${auth.token}`,
      'Cookie': auth.cookies || '',
  ...
  if (!summaryResponse.ok) {
    throw new Error(`Failed to get logs summary: ${summaryResponse.statusText}`);
  }

  const summaryData = await summaryResponse.json();
  log(`✅ Logs summary retrieved:`, colors.green);
  log(`   Overall Statistics:`, colors.bright);
  log(`   - Total AI tokens: ${summaryData.summary?.overall?.totalAiTokens || 0}`, colors.white);
  log(`   - Total AI cost: $${summaryData.summary?.overall?.totalAiCost || 0}`, colors.white);
  log(`   - Total terminal commands: ${summaryData.summary?.overall?.totalTerminalCommands || 0}`, colors.white);
  log(`   - Error rate: ${summaryData.summary?.overall?.errorRate?.toFixed(2) || 0}%`, colors.white);
...
async function runTests() {
  ...
  try {
    ...
    log(`- Assistant logging: ${assistantLogs ? 'Working' : 'Failed'}`, colors.white);
    log(`- Terminal logging: ${terminalLogs ? 'Working' : 'Not available'}`, colors.white);
