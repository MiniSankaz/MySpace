...
import { dbManager, ConnectionStatus } from '../src/core/database/connection-manager';
...
async function testDatabaseConnection() {
  ...
  try {
    ...
    log(`Initial status: ${status}`, status === ConnectionStatus.CONNECTED ? colors.green : colors.yellow);
    ...
    if (client) {
      ...
      log(`  - Successful connections: ${metrics.successfulConnections}`, colors.gray);
      log(`  - Failed connections: ${metrics.failedConnections}`, colors.gray);
    } else {
      log('✗ Database connection failed - running in offline mode', colors.yellow);
    }
  } catch (error) {
...
async function testUserServiceFallback() {
  ...
  try {
    ...
    if (user) {
      ...
      log(`  - Source: ${dbManager.isAvailable() ? 'Database' : 'Offline/Mock'}`, colors.gray);
      log(`  - User: ${user.displayName || user.username} (${user.email})`, colors.gray);
    } else {
    ...
    if (userList && userList.users) {
      ...
      log(`  - Source: ${dbManager.isAvailable() ? 'Database' : 'Mock data'}`, colors.gray);
    } else {
...
async function simulateDatabaseFailure() {
  ...
  try {
    ...
    const status = dbManager.getStatus();
    log(`Database status: ${status}`, status === ConnectionStatus.OFFLINE ? colors.green : colors.red);
    ...
    log(`Offline store status: ${isOffline ? 'OFFLINE' : 'ONLINE'}`, isOffline ? colors.yellow : colors.green);
...
async function runAllTests() {
  ...
  log(`Offline mode enabled: ${developmentConfig.enableOfflineMode}`, colors.gray);
  log(`Mock data enabled: ${developmentConfig.enableMockData}`, colors.gray);
  log(`Database fallback enabled: ${developmentConfig.enableDatabaseFallback}`, colors.gray);
