...
const TEST_COOKIE = 'accessToken=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJ0ZXN0LXVzZXIiLCJpYXQiOjE2MDAwMDAwMDAsImV4cCI6OTk5OTk5OTk5OX0.test-signature';
...
async function testAPI() {
  ...
  try {
    ...
    const listResponse = await fetch(`${BASE_URL}/api/terminal/list?projectId=${PROJECT_ID}`, {
      headers: {
        'Cookie': TEST_COOKIE
      }
    });
    ...
    const createSystemResponse = await fetch(`${BASE_URL}/api/terminal/create`, {
      method: 'POST',
      headers: {
    ...
    const createClaudeResponse = await fetch(`${BASE_URL}/api/terminal/create`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': TEST_COOKIE
    ...
    const listAfterResponse = await fetch(`${BASE_URL}/api/terminal/list?projectId=${PROJECT_ID}`, {
      headers: {
        'Cookie': TEST_COOKIE
      }
    });
    ...
    if (createSystemResult.success && createClaudeResult.success) {
      ...
      if (listAfterResult.sessions && listAfterResult.sessions.length > 0) {
        ...
        listAfterResult.sessions.forEach((session, index) => {
          console.log(`   ${index + 1}. ${session.type} terminal - ${session.tabName} (${session.id})`);
        });
      } else {
        console.log('❌ Terminal sessions NOT returned by list API');
        console.log('   This indicates a problem with the list endpoint or data format');
      }
    } else {
