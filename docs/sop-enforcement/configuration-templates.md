...
# API Configuration
API_BASE_URL=http://localhost:4110
API_TIMEOUT=30000
...
export const terminalRefactorConfig: TerminalRefactorConfig = {
  ...
  memory: {
    thresholds: {
      ...
      heapWarning: parseInt(process.env.MEMORY_HEAP_WARNING || "4110"),
    },
    cleanup: {
...
export function getApiEndpoint(
  endpoint: keyof typeof terminalRefactorConfig.api.endpoints,
): string {
...
export function validateTerminalRefactorConfig(): ValidationResult {
  ...
  if (
    terminalRefactorConfig.security.authRequired &&
    !terminalRefactorConfig.security.jwtSecret
  ) {
...
const healthCheck = fetch("http://localhost:4110/api/health");
```
...
import {
  getWebSocketUrl,
  getApiEndpoint,
} from "@/config/terminal-refactor.config";
...
export function getTestWebSocketUrl(type: "system" | "claude"): string {
  const port =
    type === "system"
      ? testConfig.websocket.systemPort
      : testConfig.websocket.claudePort;
...
data:
  ...
  CLAUDE_WS_PORT: "4002"
  WS_HOST: "0.0.0.0"
  API_BASE_URL: "http://terminal-api-service:4110"
  NODE_ENV: "production"
...
HARDCODED_PORTS=$(grep -r "4001\|4002\|4100\|5555" --exclude-dir=node_modules --exclude="*.md" . || true)
