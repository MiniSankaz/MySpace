...
class InMemoryTerminalService extends EventEmitter {
  setSessionFocus(sessionId: string, focused: boolean): void {
    ...
    this.emit('focus_changed', {
      ...
      sessionId,
      focused,
      focusedSessions: Array.from(this.focusedSessions.get(session.projectId) || []),
      timestamp: Date.now()
    });
  }
}
...
interface ServerMessage {
  type: 'stream' | 'focus_update' | 'connection_status' | 'error' | 'sync_complete';
  sessionId: string;
  projectId?: string;
  data?: any;
...
export class InMemoryTerminalService extends EventEmitter {
  ...
  public setSessionFocus(sessionId: string, focused: boolean): void {
    ...
    console.log(`[InMemoryTerminalService] Session ${sessionId} focus set to ${focused}, total focused in project: ${projectFocused.size}`);
  }
...
class TerminalWebSocketServer {
  constructor() {
    ...
    this.memoryService = require('../../dist/services/terminal-memory.service.js').inMemoryTerminalService;
  ...
  handleFocusChanged({ projectId, sessionId, focused, focusedSessions, timestamp, version }) {
    console.log(`[TerminalWS] Focus changed for project ${projectId}: session ${sessionId} = ${focused}`);
  ...
  notifyProjectClients(projectId, message) {
    ...
    console.log(`[TerminalWS] Notified ${projectSessions.size} clients in project ${projectId}`);
  }
  ...
  async handleConnection(ws, request) {
    ...
    const success = await this.registerSessionWithRetry(sessionId, ws, projectId);
    if (!success) {
  ...
  async registerSessionWithRetry(sessionId, ws, projectId, maxRetries = 5) {
    const retryDelays = [100, 500, 1000, 2000, 4000]; // Progressive backoff
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      if (session) {
        ...
        const focusedSessions = this.memoryService.getFocusedSessions(projectId);
        this.focusedSessions.set(projectId, new Set(focusedSessions));
        
        console.log(`[TerminalWS] Session ${sessionId} registered successfully on attempt ${attempt + 1}`);
        return true;
      }
      ...
      if (attempt < maxRetries - 1) {
        ...
        console.log(`[TerminalWS] Session ${sessionId} not found, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
    
    console.error(`[TerminalWS] Failed to register session ${sessionId} after ${maxRetries} attempts`);
    return false;
  }
  ...
  handleMessage(sessionId, message) {
    ...
    switch (type) {
      ...
      case 'sync_request':
        ...
        const session = this.sessions.get(sessionId);
        if (session) {
          const focusedSessions = this.memoryService.getFocusedSessions(session.projectId);
      ...
      case 'heartbeat':
        ...
        if (heartbeatSession) {
          heartbeatSession.ws.send(JSON.stringify({
            type: 'heartbeat',
            sessionId,
            timestamp: Date.now()
          }));
