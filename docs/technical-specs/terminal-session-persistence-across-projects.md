  ...
  handleProjectResumption(projectId, sessionCount) {
    ...
    sessionIds.forEach((sessionId) => {
      ...
      if (session && suspensionData) {
        ...
        if (
          this.memoryService &&
          this.memoryService.isSessionFocused(sessionId)
        ) {
  ...
  private handleMessage(message: WebSocketMessage) {
    switch (message.type) {
      case "projectSuspended":
        ...
        console.log(
          `Terminal session ${message.sessionId} suspended for project switch`,
        );
      ...
      case "projectResumed":
        ...
        console.log(
          `Terminal session ${message.sessionId} resumed after ${message.suspendedDuration}ms`,
        );
      ...
      case "sessionRestored":
        console.log(
          `Terminal session ${message.sessionId} restored from persistence`,
        );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-sm text-slate-300">{session.tabName}</span>
        {getStatusIndicator()}
      </button>
...
const ProjectSwitchNotification: React.FC<ProjectSwitchNotificationProps> = ({
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShow(false), 4100);
          ...
          <div>
            ...
            <div className="text-xs text-slate-400 mt-1">
              {previousProject && suspendedSessionCount > 0 && (
                <div>
                  • {suspendedSessionCount} terminals suspended in{" "}
                  {previousProject.name}
                </div>
              )}
              {resumedSessionCount > 0 && (
                <div>
                  • {resumedSessionCount} terminals resumed in{" "}
                  {currentProject.name}
                </div>
              )}
              {resumedSessionCount === 0 && (
                <div>
                  • No previous terminals found for {currentProject.name}
                </div>
              )}
            </div>
          </div>
...
class SessionResourceManager {
  ...
  async enforceResourceLimits(
    ...
    if (projectSessions.length >= this.MAX_SESSIONS_PER_PROJECT) {
      throw new Error(
        `Project session limit reached (${this.MAX_SESSIONS_PER_PROJECT})`,
      );
    }
...
class TerminalSecurityService {
  ...
  private logSecurityViolation(
    ...
    console.error(
      `[Security] User ${userId} attempted unauthorized ${action} on session ${sessionId}`,
    );
...
class SessionOwnershipService {
  ...
  validateSessionOwnership(
    ...
    if (ownership.ipAddress && ownership.ipAddress !== currentIP) {
      console.warn(
        `[Security] IP change detected for session ${sessionId}: ${ownership.ipAddress} -> ${currentIP}`,
      );
...
class CommandSecurityService {
  ...
  private logDangerousCommand(command: string, sessionId: string): void {
    console.error(
      `[Security] Dangerous command blocked: ${command} in session ${sessionId}`,
    );
...
class MemoryMonitor {
  ...
  private checkMemoryUsage(): void {
    ...
    // Log metrics
    console.log(
      `[Memory] Heap: ${metrics.heapUsed}MB, Sessions: ${metrics.sessionCount}, Per Session: ${metrics.memoryPerSession}KB`,
    );
    ...
    if (metrics.heapUsed > 500) {
      // 500MB threshold
      console.warn(
        `[Memory] High memory usage detected: ${metrics.heapUsed}MB`,
      );
...
curl -f http://localhost:4110/api/terminal/health || {
