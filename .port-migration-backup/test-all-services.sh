...
response=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:4000/api/v1/users/health" 2>/dev/null || echo "000")
...
response=$(curl -s "http://localhost:4000/api/v1/stocks/trending" 2>/dev/null | jq -r '.success' 2>/dev/null || echo "false")
...
portfolio_response=$(curl -s -X POST "http://localhost:4500/api/v1/portfolios" \
...
if echo "$portfolio_response" | jq -r '.success' 2>/dev/null | grep -q "true"; then
    ...
    if [ ! -z "$portfolio_id" ]; then
        read_response=$(curl -s "http://localhost:4500/api/v1/portfolios/$portfolio_id" \
