...
if (majorVersion >= 22) {
  console.warn(
    "⚠️  Warning: Node.js v22+ may have issues. Consider using Node.js v18.",
  );
}
...
app.prepare().then(async () => {
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        const wsV2Module = await requireTypeScript(
          "./src/server/websocket/terminal-ws-v2.ts",
        );
  ...
  server.listen(port, hostname, (err) => {
    ...
    console.log(
      `> Migration: ${process.env.TERMINAL_MIGRATION_MODE || "progressive"}`,
    );
    ...
    if (terminalWSV2)
      console.log("  - ws://localhost:4000/ws/terminal-v2 (new)");
