...
RESPONSE1=$(curl -s -X POST http://localhost:4000/api/terminal/create \
...
RESPONSE2=$(curl -s -X POST http://localhost:4000/api/terminal/create \
  -H "Content-Type: application/json" \
  -d '{
    "projectId": "test-project-001",
    "projectPath": "/tmp/test-project",
...
RESPONSE3=$(curl -s -X POST http://localhost:4000/api/terminal/create \
  -H "Content-Type: application/json" \
  -d '{
    "projectId": "test-project-001",
    "projectPath": "/tmp/test-project",
...
LIST_RESPONSE=$(curl -s http://localhost:4000/api/terminal/list?projectId=test-project-001)
...
STORAGE_INFO=$(curl -s http://localhost:4000/api/terminal/storage-info)
...
if [ ! -z "$SESSION_ID2" ]; then
    FOCUS_RESPONSE=$(curl -s -X POST http://localhost:4000/api/terminal/focus \
      -H "Content-Type: application/json" \
      -d "{
