...
health_check() {
    if [ "$HEALTH_CHECK_ENABLED" = "true" ]; then
        ...
        HEALTH_RESPONSE=$(curl -s http://localhost:4000/api/terminal/health || echo "failed")
...
display_summary() {
    ...
    echo "  2. Check health: curl http://localhost:4000/api/terminal/health" | tee -a "$DEPLOYMENT_LOG"
    echo "  3. View metrics: curl http://localhost:4000/api/terminal/storage-info" | tee -a "$DEPLOYMENT_LOG"
