...
async function testParallelTerminals() {
  ...
  try {
    ...
    console.log(
      `✅ Created terminals: ${terminal1.id}, ${terminal2.id}, ${terminal3.id}\n`,
    );
    ...
    console.log(
      `✅ Terminals persisted: ${terminalsAfter.length} total terminals\n`,
    );
...
async function createTerminal(type, tabName) {
  try {
    ...
    console.error(
      `Failed to create ${type} terminal:`,
