...
async function runTests() {
  ...
  const listEmpty = await testAPI(
    "GET",
    `/api/terminal/list?projectId=${PROJECT_ID}`,
  );
  ...
  if (claudeSessionId) {
    ...
    const closeSession = await testAPI(
      "DELETE",
      `/api/terminal/close/${claudeSessionId}`,
    );
  ...
  const listAfterClose = await testAPI(
    "GET",
    `/api/terminal/list?projectId=${PROJECT_ID}`,
  );
  if (listAfterClose.success) {
    console.log(
      `✅ Found ${listAfterClose.data.sessions.length} sessions after close`,
    );
  }
  ...
  const listFinal = await testAPI(
    "GET",
    `/api/terminal/list?projectId=${PROJECT_ID}`,
  );
  if (listFinal.success) {
    console.log(
      `✅ Found ${listFinal.data.sessions.length} sessions after cleanup`,
    );
  }
