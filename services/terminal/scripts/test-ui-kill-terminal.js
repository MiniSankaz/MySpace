...
async function testSystemTerminal() {
  return new Promise((resolve) => {
    ...
    const ws = new WebSocket(
      `${SYSTEM_WS_URL}?sessionId=${sessionId}&projectId=${TEST_PROJECT_ID}`,
    );
    ...
    ws.on("message", (data) => {
      ...
      if (
        msg.type === "stream" &&
        msg.data.includes("System Terminal Active")
      ) {
...
async function testClaudeTerminal() {
  return new Promise((resolve) => {
    ...
    const ws = new WebSocket(
      `${CLAUDE_WS_URL}?sessionId=${sessionId}&projectId=${TEST_PROJECT_ID}`,
    );
    ...
    ws.on("message", (data) => {
      ...
      if (
        msg.type === "stream" &&
        msg.data.includes("Claude Terminal Active")
      ) {
        console.log("✅ Command executed successfully");
      }
    });
...
async function runTests() {
  ...
  console.log(
    `System Terminal Kill: ${testResults.systemTerminal ? "✅ PASSED" : "❌ FAILED"}`,
  );
  console.log(
    `Claude Terminal Kill: ${testResults.claudeTerminal ? "✅ PASSED" : "❌ FAILED"}`,
  );
  ...
  if (allPassed) {
    ...
    console.log(
      "2. Verify handleCloseSession calls multiplexer.closeSession()",
    );
