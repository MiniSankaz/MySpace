...
function createTerminalConnection(sessionId, projectId, type = "system") {
  const port =
    type === "system"
      ? TEST_CONFIG.terminalServerPort
      : TEST_CONFIG.claudeServerPort;
  ...
  ws.on("message", (data) => {
    try {
      ...
      switch (message.type) {
        ...
        case "stream":
          console.log(
            `📨 ${terminal.id}: Output received (${message.data.length} chars)`,
          );
        ...
        case "exit":
          console.log(
            `🚪 ${terminal.id}: Process exited with code ${message.code}`,
          );
          break;
      }
    } catch (error) {
      console.error(
        `❌ ${terminal.id}: Failed to parse message:`,
        error.message,
      );
    }
  });
...
function printTestResults() {
  ...
  console.log(
    `🔌 Total Connections Attempted: ${testResults.totalConnections}`,
  );
  console.log(
    `✅ Successful Connections: ${testResults.successfulConnections}`,
  );
  ...
  console.log(
    `🔄 Background Activity Events: ${testResults.backgroundActivity}`,
  );

  const connectionRate =
    (testResults.successfulConnections / testResults.totalConnections) * 100;
  ...
  activeSockets.forEach((terminal) => {
    console.log(
      `  ${terminal.id}: ${terminal.messagesReceived} messages received, ` +
  ...
  const criteria = [
    {
      ...
      passed:
        testResults.successfulConnections === testResults.totalConnections,
    ...
    },
    {
      ...
      passed:
        activeSockets.filter((t) => t.messagesReceived > 0).length >=
        TEST_CONFIG.numTerminals,
