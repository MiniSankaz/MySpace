...
async function testMigrationService() {
  await runTest("Migration Service - Get Status", async () => {
    ...
    console.log(
      "  Features enabled:",
      Object.entries(status.featureFlags)
        .filter(([_, v]) => v)
        .map(([k]) => k)
        .join(", "),
    );
...
async function testProjectSwitching() {
  await runTest("Project Switching - Suspend/Resume", async () => {
    ...
    if (suspendedCount !== 2)
      throw new Error(`Expected 2 suspended, got ${suspendedCount}`);

    // Resume project A
    const resumedSessions = await orchestrator.resumeProject("project-a");
    console.log("  Resumed:", resumedSessions.length, "sessions");

    if (resumedSessions.length !== 2)
      throw new Error(`Expected 2 resumed, got ${resumedSessions.length}`);
...
async function testMemoryManagement() {
  await runTest("Memory Pool - Session Reuse", async () => {
    ...
    console.log(
      "  Initial memory:",
      Math.round(initialMetrics.memory.heapUsed / 1024 / 1024) + "MB",
    );
    console.log(
      "  Final memory:",
      Math.round(finalMetrics.memory.heapUsed / 1024 / 1024) + "MB",
    );

    // Check that memory didn't grow too much
    const memoryGrowth =
      finalMetrics.memory.heapUsed - initialMetrics.memory.heapUsed;
