...
class PerformanceTest {
  ...
  private displayConfig(): void {
    ...
    console.log(
      ...
      chalk.blue(this.config.operations),
    );
    console.log(
      chalk.gray("  Concurrent:"),
      this.config.concurrent ? chalk.yellow("Yes") : chalk.gray("No"),
    );
    console.log();
  }
  ...
  private async testSessionCreation(): Promise<void> {
    ...
    const sessionsPerProject = Math.ceil(
      this.config.sessions / this.config.projects,
    );
    ...
    spinner.succeed(
      `Created ${this.sessionIds.length} sessions - Avg: ${this.getAvg(this.results.metrics.createTime).toFixed(2)}ms`,
    );
  }
  ...
  private async createSession(projectId: string, index: number): Promise<void> {
    ...
    try {
      ...
      if (this.config.verbose && index % 10 === 0) {
        console.log(
          chalk.gray(
            `  Created session ${session.id} in ${duration.toFixed(2)}ms`,
          ),
        );
      }
    } catch (error) {
  ...
  private async testSessionReading(): Promise<void> {
    ...
    const readOps = Math.min(
      this.config.operations,
      this.sessionIds.length * 2,
    );
    ...
    spinner.succeed(
      `Read ${readOps} sessions - Avg: ${this.getAvg(this.results.metrics.readTime).toFixed(2)}ms`,
    );
  }
  ...
  private async testSessionUpdates(): Promise<void> {
    ...
    spinner.succeed(
      `Updated ${updateOps} sessions - Avg: ${this.getAvg(this.results.metrics.updateTime).toFixed(2)}ms`,
    );
  }
  ...
  private async testProjectSwitching(): Promise<void> {
    if (this.projectIds.length < 2) {
      console.log(
        chalk.yellow("Skipping project switch test (need at least 2 projects)"),
      );
      return;
    }
    ...
    spinner.succeed(
      `Switched ${switchOps} times - Avg: ${this.getAvg(this.results.metrics.switchTime).toFixed(2)}ms`,
    );
  }
  ...
  private async testSessionDeletion(): Promise<void> {
    ...
    spinner.succeed(
      `Deleted ${deleteOps} sessions - Avg: ${this.getAvg(this.results.metrics.deleteTime).toFixed(2)}ms`,
    );
  }
  ...
  private async cleanup(): Promise<void> {
    ...
    this.results.memory.leaked = Math.max(
      0,
      this.results.memory.final - this.results.memory.initial,
    );
  ...
  private displayResults(): void {
    ...
    console.log(
      chalk.gray("  Initial:"),
      `${(this.results.memory.initial / 1024 / 1024).toFixed(2)}MB`,
    );
    console.log(
      chalk.gray("  Peak:"),
      `${(this.results.memory.peak / 1024 / 1024).toFixed(2)}MB`,
    );
    console.log(
      chalk.gray("  Final:"),
      `${(this.results.memory.final / 1024 / 1024).toFixed(2)}MB`,
    );
    console.log(
      chalk.gray("  Leaked:"),
      this.results.memory.leaked > 10 * 1024 * 1024
        ? chalk.red(
            `${(this.results.memory.leaked / 1024 / 1024).toFixed(2)}MB`,
          )
        : chalk.green(
            `${(this.results.memory.leaked / 1024 / 1024).toFixed(2)}MB`,
          ),
    ...
    console.log(
      ...
      `${(this.results.summary.totalTime / 1000).toFixed(2)}s`,
    );
    console.log(
      chalk.gray("  Ops/Second:"),
      chalk.blue(this.results.summary.opsPerSecond.toFixed(0)),
    );
    console.log(
    ...
    if (isExcellent) {
      ...
      console.log(
        chalk.red.bold("  ⚠️  NEEDS IMPROVEMENT - Review configuration"),
      );
    }
  }
