...
async function cleanupDatabaseSessions() {
  ...
  try {
    ...
    const migratedSessions = await prisma.terminalSession.findMany({
      where: {
        OR: [
          { metadata: { path: ["legacyId"], not: { equals: null } } },
          { metadata: { path: ["migratedAt"], not: { equals: null } } },
          { metadata: { path: ["migratedFrom"], not: { equals: null } } },
        ],
      },
    });
    ...
    for (const [projectId, sessions] of projectSessions.entries()) {
      if (sessions.length > 100) {
        console.log(
          `⚠️  Project ${projectId} has ${sessions.length} sessions!`,
        );
    ...
    for (const projectId of problematicProjects) {
      ...
      console.log(
        `Deleting ${sessionsToDelete.length} old sessions for project ${projectId}`,
      );

      for (const session of sessionsToDelete) {
        await prisma.terminalSession
          .delete({
            where: { id: session.id },
          })
          .catch((err) => {
            console.error(
              `Failed to delete session ${session.id}:`,
              err.message,
            );
          });
      }
...
async function cleanupLocalStorage() {
  ...
  try {
    ...
    if (exists) {
      ...
      console.log(
        `Found ${data.sessions?.length || 0} sessions in local storage`,
      );
        ...
        if (data.projectSessions) {
          for (const [projectId, sessionIds] of data.projectSessions) {
            if (Array.isArray(sessionIds) && sessionIds.length > 20) {
              console.log(
                `Trimming project ${projectId} from ${sessionIds.length} to 20 sessions`,
              );
