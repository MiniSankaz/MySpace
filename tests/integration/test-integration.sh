...
echo "----------------------------------------"
test_endpoint "Gateway Health" "http://localhost:$PORT_GATEWAY_MAIN/health" "200"
test_endpoint "User Management Health" "http://localhost:$PORT_SERVICE_USER/health" "200"
test_endpoint "Portfolio Service Health" "http://localhost:$PORT_SERVICE_PORTFOLIO/health" "200"
echo ""

echo -e "${BLUE}2. Testing Service Discovery${NC}"
echo "----------------------------------------"
test_endpoint "Service Registry" "http://localhost:$PORT_GATEWAY_MAIN/services" "200"
test_json_response "Registry Success" "http://localhost:$PORT_GATEWAY_MAIN/services" ".success" "true"
echo ""

echo -e "${BLUE}3. Testing Direct Service Access${NC}"
echo "----------------------------------------"
test_endpoint "Portfolio Stocks Direct" "http://localhost:$PORT_SERVICE_PORTFOLIO/api/v1/stocks/trending" "200"
test_endpoint "User Management Direct" "http://localhost:$PORT_SERVICE_USER/info" "200"
echo ""
...
test_endpoint "Stocks Search via Gateway" "http://localhost:$PORT_GATEWAY_MAIN/api/v1/stocks/search?q=AAPL" "200"
test_endpoint "Trending Stocks via Gateway" "http://localhost:$PORT_GATEWAY_MAIN/api/v1/stocks/trending" "200"

# User service routes through gateway
test_endpoint "Auth Login via Gateway" "http://localhost:$PORT_GATEWAY_MAIN/api/v1/auth/login" "400" "POST" '{"email":"test@example.com","password":"test"}'
echo ""
...
test_endpoint "Create Portfolio" "http://localhost:$PORT_SERVICE_PORTFOLIO/api/v1/portfolios" "201" "POST" "$portfolio_data"

# Get portfolios
test_endpoint "List Portfolios" "http://localhost:$PORT_SERVICE_PORTFOLIO/api/v1/portfolios" "200"

# Search stocks
test_endpoint "Search AAPL Stock" "http://localhost:$PORT_SERVICE_PORTFOLIO/api/v1/stocks/search?q=AAPL" "200"
test_json_response "AAPL Symbol" "http://localhost:$PORT_SERVICE_PORTFOLIO/api/v1/stocks/search?q=AAPL" ".data[0].symbol" "AAPL"
echo ""
...
test_endpoint "Register User" "http://localhost:$PORT_SERVICE_USER/auth/register" "201" "POST" "$register_data"

# Login attempt
login_data='{"email":"test@example.com","password":"Test@123"}'
test_endpoint "Login Attempt" "http://localhost:$PORT_SERVICE_USER/auth/login" "401" "POST" "$login_data"
echo ""
...
response_headers=$(curl -s -I -H "X-Correlation-Id: $correlation_id" "http://localhost:$PORT_GATEWAY_MAIN/health" 2>/dev/null)
...
for i in {1..5}; do
    curl -s -o /dev/null "http://localhost:$PORT_GATEWAY_MAIN/health" 2>/dev/null
done
response=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PORT_GATEWAY_MAIN/health" 2>/dev/null)
...
echo "----------------------------------------"
test_endpoint "Invalid Route" "http://localhost:$PORT_GATEWAY_MAIN/api/v1/invalid" "404"
test_endpoint "Invalid Service Route" "http://localhost:$PORT_SERVICE_PORTFOLIO/invalid" "404"
echo ""
