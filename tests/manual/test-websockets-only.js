      ...
      try {
        ...
        if (message.type === "connected") {
          ...
          if (
            ...
            message.data.includes("Starting Claude")
          ) {
            claudeStarted = true;
...
async function testMultipleConnections() {
  ...
  try {
    ...
    console.log(
      `📊 Multiple connections result: ${successful.length}/4 successful`,
    );
...
function generateReport() {
  ...
  console.log(
    `   📈 Success Rate: ${((passedTests.length / allTests.length) * 100).toFixed(1)}%`,
  );
