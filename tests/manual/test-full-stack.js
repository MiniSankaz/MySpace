...
// Test configurations
const SERVICES = {
  gateway: "http://${getGatewayPort()}",
  portfolio: "http://${getServiceUrl("portfolio")}",
  ai: "http://localhost:4201",
  terminal: "http://${getServiceUrl("terminal")}",
  frontend: "http://${getGatewayPort()}",
};
...
async function testFullStackIntegration() {
  ...
  console.log(`   WebSocket: ws://${getGatewayPort()}/ws`);
