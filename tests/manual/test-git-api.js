...
async function testGitAPI() {
  ...
  const baseUrl = "http://${getGatewayPort()}";
  ...
  try {
    ...
    const statusRes = await fetch(
      `${baseUrl}/api/workspace/git/status?projectId=${projectId}`,
      {
        credentials: "include",
      },
    );
  ...
  try {
    ...
    const branchesRes = await fetch(
      `${baseUrl}/api/workspace/git/branches?projectId=${projectId}`,
      {
        credentials: "include",
      },
    );
  ...
  try {
    ...
    const apiDir = require("fs").readdirSync(
      "/Users/sem4pro/Stock/port/src/app/api/workspace/git",
    );
