...
console.log(
  `   - Total Heap Size Executable: ${toMB(heapStats.total_heap_size_executable)}MB`,
);
console.log(
  `   - Total Physical Size: ${toMB(heapStats.total_physical_size)}MB`,
);
console.log(
  `   - Total Available Size: ${toMB(heapStats.total_available_size)}MB`,
);
...
if (maxHeapMB < 7900) {
  console.log(
    `   ❌ WARNING: Heap limit is only ${maxHeapMB}MB, expected ~8192MB (8GB)`,
  );
...
console.log(
  `   - Process Arguments: ${process.execArgv.join(" ") || "(none)"}`,
);
...
console.log(
  "   - heap_size_limit: Maximum heap size configured (--max-old-space-size)",
);
